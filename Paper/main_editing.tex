...
Our experiments used a common physical setup: a small wheeled robot operating inside a walled arena, under an overhead camera system that tracked its pose (Fig.\ref{fig_robot}). The arena was approximately 3.5 by 3.5 m. The backing of the walls was formed by cardboard panels, each about 80 cm wide and about 30 cm high. Onto the cardboard, we glued foam tiles sold as decorative background for terraria. These panels had a variety of surface textures simulating different types of natural rock. The tiles were fixed to the cardboard with 5 cm gaps between them. The gaps and the textured tile surfaces together produced complex, overlapping reflections.
...
The arena walls were lightweight and could be repositioned rapidly between data-collection runs. Sub-assemblies built from four panels fixed at right angles were also used as freestanding rectangular obstacles inside the arena. In addition to the panel walls and panel sub-assemblies, we used vertical poles as a second class of in-arena obstacle. We used wooden dowels (25 mm diameter, ~60 cm height) as poles, each held upright by a low-profile metal base. We could place poles anywhere within the arena. The arena floor consisted of interlocking high-density foam tiles with a smooth top surface, providing the wheels with reliable traction and a uniform background for the overhead tracker.


We used a Pololu 3pi+~2040 robot (Fig.\ref{fig_robot}) (Pololu Robotics \& Electronics, Las Vegas, NV), a commercially available differential-drive platform built around an RP2040 microcontroller (diameter: 96 mm). We added a HiLetgo ESP8266 ES Wi-Fi module so that all higher-level control logic could run off-board on a host computer, while the robot executed low-level motion and acoustic commands on request. Communication with the host was by JSON-framed messages over a TCP link.
...
We trained the inverse model on echoes collected as the robot drove through the arena, with training data collected in five sessions. For each session, the arena's overall outline was similar, dictated by the overhead camera system's field of view. By moving some walls and installing interior walls, we altered the layout locally. In addition, we placed a number of poles in the arenas at semi-random positions. This ensured we collected data across a range of arena geometries.
...
For each session, we used a predetermined number of semi-random positions within the arena (see Fig.\ref{fig:arena_layouts} for an example). We sequentially drove the robot to each position using the overhead camera system. Once the robot arrived at a position, it rotated to five headings spaced $72^{\circ}$ apart. Modeling the emitter as a 15\,mm-diameter circular piston at 42\,kHz, its $-6$\,dB main lobe is about $\pm 22^{\circ}$ wide and at $\pm 35^{\circ}$ off axis its directivity has fallen by roughly 18\,dB. Because successive headings are $72^{\circ}$ apart, well beyond this main lobe, the five emissions sampled approximately non-overlapping, and therefore approximately independent, regions of the environment. At each assumed heading, the robot emitted one sonar pulse and recorded the resulting echo envelopes at the left and right ear.

Across the five sessions, we collected about 2100 binaural echoes. For perspective, \citet{Barchi2013} flew \animal{ef} in an arena of about 6 by 3 meters. These animals emitted calls at 10--20 per second. At this rate, 2100 calls would take less than 4 minutes to collect, suggesting bats could trivially gather the required training data.

For each robot position and orientation, we also collected a visual snapshot using the overhead camera system, recording the local geometry. We modeled our robotic bat with a visual field of $\pm35^{\circ}$ for two reasons. First, this coincides with the approximate width from which echoes from the walls and poles were received. Thus, it matched the sonar system's field. Second, a field of this width is biologically plausible for a bat, lying between the binocular ($50^{\circ}$) and monocular ($120^{\circ}$) visual fields measured in the gleaning bat \animal{mc} \citep{Bell1986}. Within this field, we extracted the nearest reflector from manual annotations marking walls and pole locations on the overhead arena view. This nearest reflector set the ping's class label: \emph{wall}, \emph{pole}, or \emph{none}. The \emph{none} label indicated no reflector was within the cone or that the nearest reflector lay beyond 1\,m. For a pole, we recorded azimuth (relative to the robot's heading, as a signed angle) and range (to the pole's surface rather than its center). For a wall, we recorded a coarse three-direction depth profile: the $\pm 35^{\circ}$ cone was divided into left, center, and right thirds, each about $23^{\circ}$ wide, and the nearest wall distance within each third gave that direction's depth, resulting in a very coarse depth profile.

\subsubsection{Inverse-model architecture and training}

Our inverse model is a convolutional network (Fig.~\ref{fig:network}). A shared trunk of one-dimensional convolutions, pooling, and a fully connected layer encodes each ear's envelope into a 32-element embedding ($z_L$ and $z_R$). Four output heads use these embeddings: \emph{classification} (wall, pole, or none), \emph{depth} (the three wall distances), \emph{azimuth} (the pole's azimuth), and \emph{range} (the pole's distance). The last three are heteroscedastic regression heads: each returns both a value and a variance, giving every distance and bearing an uncertainty estimate.
...
\end{equation}
%
Each of the four heads is run twice, once using $z_{LR}$ as input and once using $z_{RL}$ as input. We then averaged the outputs after flipping the depth-head output and the azimuth-head sign for $z_{RL}$. The class and range heads require no correction, as their outputs are unchanged by the swap; we averaged them directly. The predicted variances are combined symmetrically, in the same way as the depth means. Encoding symmetry this way forces the model to respect it exactly. It also roughly doubles the effective training data (each emission serves as its mirror image) and reduces the parameter count. This symmetry constraint is not strictly necessary: a variant without it, free to learn left-right asymmetries, fit the depth profile slightly worse (data not shown). Table~\ref{tab:inverse-params} lists the configuration.
...
We optimized with Adam (learning rate $10^{-3}$, batch size 64) for 60 epochs. For the first 20 epochs, we fitted the regression means with a plain squared-error loss before enabling the variance terms, which stabilized early training. We rescaled three quantities for training: standardized input envelopes, pole-azimuth targets divided by the cone half-angle, and pole-range targets divided by the 1\,m maximum labeled range, placing all regression targets on a common scale.


We fitted the inverse model on data from all five sessions. From each session, we held out 15% of the data in contiguous sets—the data collected nearest a randomly chosen location. Training used early stopping on these regions, and we report performance on both training and held-out data (Table~\ref{tab:inverse-results}). Because the held-out regions are coherent arena patches, they reflect locations unseen during training; a small performance gap indicates minimal overfitting. The model, trained on the remaining 85%, is the one deployed on the robot. A single held-out region of this size is a noisy estimate, particularly for the pole class (only 90 of 322 held-out echoes). As a robustness check, we also cross-validated the architecture and procedure over the four spatial quadrants, scoring each echo once while held out. We report those figures alongside the deployed model where informative. We did not use a separate validation set; instead, we consider successful task completion as model validation.

\subsection{Experiment 1: obstacle avoidance and target approach}


In Experiment 1, we asked whether the noisy inverse model could drive simple navigational behavior from sonar alone. The arena contained a single pole. The robot's task was to approach the pole while avoiding walls. The task drew on features recovered by the inverse: the nearest reflector's class, the pole's azimuth and range (when a pole is nearest), and a coarse wall-distance profile (otherwise). The task is analogous to prey-capture experiments where bats approach objects at unknown positions, e.g., tethered insects \citep{Ghose2003}, mealworms \citep{Falk2014}, or artificial flowers \citep{Simon2011}.

As a comparison, besides sonar-only trials, we ran trials using vision via the overhead camera system. At each step, regardless of modality, the robot received a single local feature description of the nearest reflector within its forward ±35\textdegree field. This description included the reflector's class and matching geometry: pole azimuth and range for poles, and a three-direction depth profile for walls. When no reflector was within range, the feature was empty. When using sonar, the feature was the inverse model's output for the binaural emission acquired at the robot's current pose in the arena. When using vision, it was read from the overhead camera's digitized arena geometry at the robot's current pose, taking the nearest reflector in the same $\pm 35^{\circ}$ cone. Crucially, vision supplied only this local feature from the robot's instantaneous pose, never a map of the arena. Although the overhead camera held the complete arena geometry, the controller could query it only within the same forward cone as the sonar inverse. Thus, the overhead camera system modeled a bat's local visual field.

Our vision model made perception stroboscopic for both modalities: the robot perceived the environment once per step. Although bat sonar is inherently stroboscopic, vision is not. We modeled vision as intermittent, however, to match the sampling rate of the two conditions. They were not matched in range, however. The sonar inverse model did not perceive objects beyond its 1\,m training range. Vision was not range-limited, reflecting the differences in operational range between sonar and vision \citep{Boonman2013}.


The mapping from local features to motor commands was hand-coded rather than learned. In animals, such mappings can be acquired through learning, as when bats are trained to search for targets over repeated trials \citep{Ghose2003,Falk2014,Simon2011,Holland2005,Finger2025}. We did not model this learning because the question is not how bats learn to use the inverse but whether the cross-modally trained inverse delivers sufficiently reliable features for such learning. The fixed rule thus stands in for the association a bat would acquire through reinforcement. Note that it maintains no memory of earlier observations and no map of the arena. Thus, nothing downstream of perception can compensate for poor estimates; whether the robot reaches the pole depends directly on the quality of the features the inverse recovers.

The robot used the extracted feature as follows. When the robot perceived no pole and either no wall or a wall further than 750\,mm away, it wandered through the arena in a slight curve, covering 150\,mm between measurements. The curvature was re-randomized on each wall contact, so that repeated encounters did not send the robot along the same arc. In lab experiments, the bat \animal{ef} flies at about 2.5 m/s while emitting 10 to 20 calls per second \citep[e.g.,][]{Barchi2013}, and so covers roughly 125 to 250 mm between successive calls. The robot's 150 mm step falls within this range. Hence, while we did not aim to model a specific species, this indicates that the distance travelled between acquisitions is biologically plausible.
