...
We trained the inverse model on data from all six sessions. From each session, we held out 15\% of the data in contiguous sets—the data collected nearest a randomly chosen location. We used early stopping on these regions and report performance on both training and held-out data (Table~\ref{tab:inverse-results}). Because the held-out regions are coherent arena patches, they reflect locations unseen during training; a small performance gap indicates minimal overfitting. We deployed on the robot the model trained on the remaining 85\%. A single held-out region of this size provides a noisy estimate, particularly for the pole class ($175$ of the $418$ held-out echoes). As a robustness check, we also cross-validated the architecture and procedure over the four spatial quadrants, scoring each echo once while held out. We report those figures alongside the deployed model where informative. We did not use a separate validation set; instead, we consider successful task completion as model validation.
...
\dnote[20]{Rewrite target for this subsection; Dieter's note from Methods 2.2.2 is folded in here. (1) Replace the aggregate metrics with a plain summary of what the model can and cannot recover: range survives to about $2.5$\,m at roughly $11\%$ error, while azimuth and class both collapse at about $1.4$\,m; the nearest-reflector head tracks to $2579$\,mm where the pole-range head saturates near $750$\,mm. (2) This is where the point about distance belongs: sonar does not give range for free. A time of flight yields a distance only when one isolated reflector produced it; with several reflectors in the beam the hard part is attributing an echo to an object, which is exactly what the two range heads separate. The model can locate what it cannot name. (3) Hedge the $1.4$\,m figure with the mechanism rather than the number: discrimination fails once a nearer reflector lies outside the analysed cone, and in our arena that holds for effectively every target beyond $1.4$\,m. Uncontested far targets barely exist there ($27$ pings beyond about $1.9$\,m), so the number is arena-bound while the mechanism is not.}
