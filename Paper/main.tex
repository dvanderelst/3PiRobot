...
We closed this point directly with a further control condition, in which we left both arenas untouched and instead corrupted what the robot sensed. At every step, we replaced the whole feature the inverse model reported with one drawn at random from a pool of $500$ predictions recorded earlier on the same path by the same controller. The substitutes are therefore realistic readings of that arena, with the statistics the robot would ordinarily encounter; what has been removed is their relation to where the robot actually was. Withholding the input instead was not an option, since every feature the inverse model reports is a claim about the world and there is no null reading to supply. We ran this condition three times in each arena and did not retrain the controller.

The route did not survive it (Fig.~\ref{fig:sensory-clamp}). All six runs ended in a collision, and none covered more than half a lap: $0.20$, $0.44$ and $0.39$ laps in Arena 1, and $0.45$, $0.47$ and $0.51$ in Arena 2, against $8.5$ and $6.4$ laps without a single contact in the corresponding intact runs. The median distance from the trained path rose from $37$\,mm to between $110$ and $298$\,mm in Arena 1, and from $43$\,mm to between $99$ and $149$\,mm in Arena 2. A controller running its route open loop would by definition be indifferent to its input, and this one is not. We read the result qualitatively only. It shows that the robot uses what it senses; it does not measure how far dead reckoning would carry it alone, because the manipulation supplies a false reading rather than no reading, so the robot may be steering on bad information rather than merely running uninformed.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=\textwidth]{fig_sensory_clamp}
	\caption{The sensory-deprivation control, in (A) Arena 1 and (B) Arena 2. The three coloured trajectories in each panel are the shuffled-input runs, in which every feature the inverse model reported was replaced by one drawn at random from a pool of $500$ predictions recorded earlier on the same path by the same controller. They are drawn against two references: the dashed line is the trained path, and the thin grey trace around it is one intact run of $500$ steps, included so that the comparison is with what the robot actually does rather than only with the path it was trained on. The width of that grey band is that single run's own variation from lap to lap, a median of $37$ and $43$\,mm from the path. Blue marks the walls and blocks and purple the poles, open circles mark the release pose, and crosses mark the last logged pose of each shuffled-input run, one step before it met a wall or a pole. The intact runs complete $8.5$ and $6.4$ laps with no contact; no shuffled-input run reached half a lap.}
