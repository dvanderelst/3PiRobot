...
In the second experiment, we asked whether the inverse model can support a more complex task. We selected a task inspired by \citet{Barchi2013}, who flew \animal{ef} in a flight cage and observed that the bats converged onto idiosyncratic but stereotypical paths. While the task is inspired by bats converging onto stable flight paths, something they also seem to do in the wild \citep{Holderied2006}, we did not directly model the experiments of \citet{Barchi2013} or behavior in the wild. In particular, we modeled a bat that has learned to fly a stable route and, while flying it, learns to associate the features its sonar recovers at each point with the motor commands that keep it on the route, i.e., those features for which it has an inverse model. Once the bat learns the association, it could fly the same route supported by sonar alone. Currently, it is unknown whether bats can transfer route following learned through vision (alone) to sonar. Although this could potentially be tested under experimental conditions, bats in the wild would always have access to sonar. However, for the purpose of this paper, i.e., demonstrating that a noisy inverse model supports non-trivial behavior, this scenario is a good test case.


To determine the path for the robot to follow, we drew a closed loop over the digitized arena, together with a rectangular release box and a release heading, and resampled it at $25$\,mm spacing so that any position could be expressed as a distance traveled along the loop. We implemented the controller as a recurrent network with $32$ hidden units. At each step, it received the same local feature the inverse model supplies in Experiment 1, together with the rotation it had commanded on the previous step, and it returned a rotation, capped at $90^{\circ}$, after which the robot drove forward $150$\,mm. Because the network is recurrent, it can combine what it senses with a record of the movements it has commanded.


We trained the controller in simulation to associate the local features of the arena with the motor commands that keep the robot on the path. We did not model how the route came to be chosen, nor where the corrections that hold the robot on it (while using vision) originate. In other words, we assumed that the bat already knew how to follow the path visually. We only modeled acquiring a controller that could mimic the path using sonar, using the features extractable by the inverse model. A pure-pursuit controller with access to the robot's true position drove the route, projecting that position onto the loop, advancing $200$\,mm along it, and returning the rotation that pointed at the resulting target. We trained the controller with no access to the robot's position. It received only the local feature and the rotation it had commanded on the previous step, so that it could learn only those associations that features recoverable from sonar can support. The features presented during training carried realistic error, drawn from the errors the inverse model makes on real echoes. We applied two kinds of perturbation during training. First, on $30\%$ of steps we disturbed the commanded rotation by about $30^{\circ}$ while keeping the clean rotation as the training target, so that the controller also learned to return to the path after leaving it. Second, the simulated motors did not execute commands exactly. Besides a small per-step error, we gave each episode a fixed gain error of up to $15\%$ in rotation and $5\%$ in distance, of the kind a real robot shows. A sustained error of this sort does not average out over a lap. Hence, the controller could not hold the path by counting its own movements alone and had to learn to compensate for long-term drift in its position from what it senses.


We then deployed the trained controller on the robot, releasing it from the box and letting it run for several laps. We compared three conditions. In the first, the arena was as we had trained the controller. In the second, we displaced a section of wall, putting what the robot senses in conflict with the layout it was trained in. This is the condition of interest: the extent to which the robot follows the displaced wall rather than holding its previous course indicates how strongly it relies on the walls as opposed to its own record of movement. In the third condition, we deployed a second controller, trained identically except that we withheld the sonar inputs, so that it received only its previous rotation. This controller can only reproduce the shape of the path from its own movements, and how far it gets is therefore a measure of what is achievable without sonar at all.
...
The model reached its lowest validation loss on the held-out region at epoch 43. The five sessions yielded 2135 labeled echoes ($53\%$ wall, $22\%$ pole, $25\%$ none), with the spatial holdout setting aside 322 ($15\%$). On the held-out region, the model achieved $80.1\%$ accuracy across the three classes, against $86.0\%$ on the training echoes (Table~\ref{tab:inverse-results}). Accuracy varied across classes. The model recovered walls—the majority and most distinctive class—well (held-out precision $88.6\%$, recall $89.8\%$), but found poles and the \emph{none} class harder (held-out recall $63.3\%$ and $81.2\%$). Because walls were classified with high accuracy, most remaining errors were confusion between poles and \emph{none} (Fig.~\ref{fig:inverse-results}A), which is expected since both correspond to the absence of a nearby wall and are separated only by a weak, often distant, pole echo.


The difference between the two columns is larger than the fold-to-fold variation of the same model and should not be interpreted as a measurement of how much the model loses off its training locations. Under the four-quadrant cross-validation described above, where we scored every echo once while holding it out, classification accuracy was $83.9\% \pm 4.0\%$ and pole recall $71.2\% \pm 11.0\%$ (mean and standard deviation across folds). The deployed model's held-out column falls inside that spread. Note that its pole figures rest on 90 echoes, so a single echo moves recall by $1.1$ points. We report the single split because it belongs to the model actually deployed, but we treat the cross-validated figures as the better estimate of the model's accuracy.


When a reflector was present, the model also recovered its geometry, although coarsely. The model recovered pole range most accurately of the three geometric quantities. In the held-out regions, the root-mean-square error was $99$\,mm (mean absolute error $72$\,mm, over $n=90$ held-out pole echoes), against $85$\,mm in-sample, and against $213$\,mm for a predictor that always returns the mean pole range (Fig.~\ref{fig:inverse-results}C). Under cross-validation, the range error was $106 \pm 11$\,mm. The model recovered pole azimuth less precisely, with a held-out root-mean-square error of $12.9^{\circ}$ (mean absolute error $9.9^{\circ}$) across the $\pm 35^{\circ}$ field, against $10.6^{\circ}$ in-sample. This ordering is expected. Range follows from the time of return and is available at either ear alone, whereas azimuth must be recovered from a comparison between the two ears, a much weaker cue given the narrowband emission and the modest ear separation. The azimuth estimate is nevertheless enough to place a pole reliably on the correct side of the robot and to provide a usable, if rough, bearing. The wall depth profile was the coarsest of the three: the model recovered held-out left, center, and right distances with root-mean-square errors of $339$, $391$, and $231$\,mm over the $1$\,m operating range (in-sample $307$, $300$, $273$\,mm). The corresponding mean absolute errors were smaller ($133$ to $212$\,mm), so the root-mean-square error is inflated by a minority of large errors rather than reflecting the typical case; the held-out center value rests on relatively few wall echoes and is noisier than the more even in-sample profile.
...
\begin{table}[ht]
\centering
\caption{Performance of the deployed inverse model on the training pings (in-sample) and on the spatially held-out $15\%$ (held-out). Accuracy, precision, and recall are percentages.}
