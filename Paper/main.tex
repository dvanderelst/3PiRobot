...
In the second experiment, we asked whether the inverse model can support a more complex task. We selected a task inspired by the stable flight paths observed in bats, both in experiments \citep{Barchi2013} and in the field \citep{Holderied2006}. In particular, we modeled a bat that has learned to fly a stable route and, while flying it, learns to associate the features its sonar recovers at each point --- those for which it has an inverse model --- with the motor commands that keep it on the route. Once the bat learns the association, it could fly the same route supported by sonar alone. Currently, it is unknown whether bats can transfer route following learned through vision (alone) to sonar. Although this could be tested under experimental conditions, bats in the wild would always have access to sonar. However, for this paper, i.e., demonstrating that a noisy inverse model supports non-trivial behavior, this hypothetical scenario is a good test case.
...
%Because the network is recurrent, it can integrate what it senses now with what it sensed earlier. The rotation fed back is the one the network commanded, not the one the motors executed: the robot has no odometry, no inertial sensor and no access to its own position (Fig.~\ref{fig:control-loop}). It does not follow that the network must rely on what it senses. The robot drives a fixed $150$\,mm at every step, so a count of steps is already a measure of distance traveled along the route, and a recurrent network can hold such a count without consulting its input at all. Whether the route rests on what the robot senses or on an internally generated sequence of this kind is therefore an empirical question, and the manipulations described below are how we ask it.
