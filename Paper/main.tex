...
\begin{figure}[!htbp]
	...
	\caption{The sensory-deprivation control, in (A) Arena 1 and (B) Arena 2. Each panel shows one intact run of $500$ steps (red) together with the three clamped runs, in which every feature the inverse model reported was replaced by one drawn at random from a pool of $500$ predictions recorded earlier on the same path by the same controller. The dashed grey line is the trained path, blue marks the walls and blocks and purple the poles, open circles mark the release pose, and crosses mark the last logged pose of each clamped run, one step before it met a wall or a pole. The intact runs complete $8.5$ and $6.4$ laps with no contact; no clamped run reached half a lap.}
