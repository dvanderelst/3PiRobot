...
Arena 1 held five rectangular blocks and a single pole, and the path was a closed loop of $8.8$\,m drawn between them (Fig.~\ref{fig:arena1-runs}). Each run of $500$ steps therefore covered about eight and a half laps.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{fig_arena1_runs}
    \caption{Arena 1: the trained route and what each change to the arena did to it. Blue marks the digitised layout, the dashed grey line is the path the controller was trained on, and red is the trajectory the robot flew. (A) The arena as trained, run twice. (B) The pole removed, marked with a cross where it stood. (C) The pole and the central block removed. (D) The central block and the one below it displaced about $320$\,mm east, drawn from their measured new positions with an arrow from where they were trained; the thicker section of the path marks the stretch those blocks border, over which the route shift is measured. The layout drawn in every panel is the trained one, which was not re-digitised for the manipulated runs, so removals and displacements are shown from the recorded measurements rather than from a fresh digitisation of each arena.}
    \label{fig:arena1-runs}
\end{figure}

Run twice without any change to the arena, the robot followed the path closely and repeatably. Its median distance from the path was $37$\,mm in both runs, with the ninetieth percentile at $89$ and $94$\,mm and the largest excursion at $188$ and $185$\,mm. The two runs are not the same trajectory, but they are the same behavior, and the agreement between them is the floor against which the changes below are read.

We then removed objects. Taking away the pole alone raised the median distance from the path to $68$\,mm, the ninetieth percentile to $174$\,mm and the largest excursion to $618$\,mm. Taking away the pole and one of the blocks raised them further, to $118$, $370$ and $639$\,mm. Both objects are therefore load-bearing: the robot does not merely pass them, it uses them.

Removal shows that an object matters, but not that its position matters, since removing it also changes the scene as a whole. We therefore displaced two of the blocks by about $320$\,mm and left everything else in place. The route moved with them. Over the stretch of the loop those blocks border, the trajectory sat $124$\,mm east and $88$\,mm north of the trained path, against $8$ and $9$\,mm in the first baseline run and $19$ and $19$\,mm in the second. The shift was present on every lap: taking the median eastward offset over that stretch lap by lap gives $156$, $80$, $148$, $111$, $171$, $133$, $158$, $125$ and $105$\,mm across the nine laps, while the same measurement on the two baseline runs scatters about zero. Away from that stretch the route was unchanged, so the effect is local to the objects that moved.

The robot therefore follows the blocks when they move, but only partway: a displacement of about $320$\,mm pulled the route by roughly a third of that. This is what one would expect of a route held by several cues at once. Two blocks moved while three blocks, the pole and all four walls stayed where they were, and the route settled between what the moved objects implied and what the stationary ones did.
