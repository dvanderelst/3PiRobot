...
\begin{figure}[!htbp]
    ...
    \caption{Arena 2: the trained route and what each change to the arena did to it, drawn as in Fig.~\ref{fig:arena1-runs}. Blue marks the digitised layout, purple the poles, the dashed grey line the trained path and red the flown trajectory. (A) The arena as trained, run twice. (B) The two northern poles removed, crossed where they stood. (C) The same two poles displaced, run twice. Dashed circles mark their trained positions and the arrows run to where each was measured in that run's own photograph, in the colour of the corresponding trajectory. (D) A section of the southern wall displaced inward. Grey marks the trained boundary and black the wall's measured position during the run, from a line fit to the changed region of the photograph. (E) Distance from the path against position around the loop, as a median in a sliding window of $500$\,mm of path, with the stretch the displaced wall borders shaded and the manipulated objects marked. Removing the poles raises the profile where they stood, displacing them changes nothing anywhere, and displacing the wall produces a single tall peak where it borders the loop.}
...
Finally we displaced a section of the southern boundary wall inward by about $450$\,mm, across the part of the loop it borders (Fig.~\ref{fig:arena2-runs}D). The route followed it. The median distance from the path rose to $83$\,mm, the ninetieth percentile to $224$\,mm and the largest excursion to $496$\,mm, and the error profile shows a single tall peak of $412$\,mm at $39\%$ of the loop, exactly the stretch the moved wall borders (Fig.~\ref{fig:arena2-runs}D,E). Over that stretch the trajectory sat $184$\,mm east and $288$\,mm north of the trained path, against $22$ and $12$\,mm north in the two baselines, so the route moved roughly two thirds of the way with the wall. The shift was present on every lap: the median northward offset over that stretch was $289$, $181$, $177$, $196$, $199$, $286$ and $198$\,mm across the seven laps, while the two baseline runs give $1$ to $31$\,mm.
