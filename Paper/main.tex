...
\begin{figure}[htb]
    ...
    \caption{Arena layouts for collecting inverse-model training data. We recorded echoes in six arenas, each sharing an overall outline set by the overhead camera's field of view. Within that outline we moved boundary walls, installed interior walls, and placed four or five poles at semi-random positions, so that the sessions sampled a range of geometries. All panels are overhead images with the digitized geometry overlaid (walls, cyan; poles, red; ping positions, yellow). Panel A shows arena 1 together with the positions at which the robot ensonified it, the sampling scheme used in every session; panels B to F show arenas 2 to 6 with their geometry alone. All panels share the scale bar (1\,m) shown in panel A.}
    \label{fig:arena_layouts}
\end{figure}

% I know that's why we did this but I am thinking this might be too much detail (for example, we don't mention other areans having boxes in them because we wanted box echoes). So, let's park this for now.: Arena 6 (F) was laid out last and served a different purpose: its interior is unobstructed, and it returns in-cone ranges beyond $3$\,m where arenas 1 to 5 reach at most about $2.1$\,m, so this session supplied the long-range echoes the earlier arenas could not produce.
...
\begin{figure}[!htbp]
    ...
    \caption{Inverse-model performance on the held-out $15\%$ of echoes. (A) Classification of the nearest reflector, as row-normalized recall: walls are recovered well, while poles and \emph{none} are mostly confused with each other. (B) Predicted versus true pole azimuth, colored by the model's predicted uncertainty $\sigma$; bearings follow the diagonal but with substantial scatter. (C) Predicted versus true pole range, colored in the same way; range is the most accurately recovered quantity, as expected for a cue available from the time of return at a single ear. (D) Predicted versus true wall distance for the three depth directions (colors); distances are accurate at close range and degrade with range. The dashed lines mark perfect prediction. (E) Fraction of echoes classified correctly, against the true range to the nearest reflector. Recall is not uniform with range: walls and poles are recovered well through the middle of the operating range and both degrade sharply in the last $100$\,mm before the $1$\,m boundary, where poles fall to $26\%$; \emph{none} is in turn least reliable just beyond it. The classes partition the range axis at $1$\,m, so the quantity plotted changes there: to the left it is recall of a reflector within the trained range, to the right it is correct abstention. Unlike (A)--(D), which use the held-out echoes, (E) uses all $2135$ so that range can be resolved in $100$\,mm bins; its wall and pole values therefore sit slightly above the held-out figures in (A). Together the panels show that the inverse recovers the target features but noisily, and that the noise has structure: it concentrates at the edge of the operating range.}
