...
Nineteen of the $20$ runs ended with the robot arriving at the pole and facing it: all ten runs using vision, and nine of the ten using sonar. No run collided with a wall or became trapped in a corner. The closest any run came to a wall was $203$\,mm using sonar and $216$\,mm using vision, above the $20$\,mm clearance at which we scored a collision. When the stop criterion fired, the robot was between $258$ and $538$\,mm from the pole when using sonar, and between $322$ and $390$\,mm using vision, according to the overhead camera system. While approaching the pole, the robot rotated on each step to regulate the perceived azimuth to zero. As a consequence, when arriving at the pole, the perceived azimuth of the pole was already within $15^{\circ}$, so no final azimuth corrections were needed in any of the $20$ runs. The overhead camera system confirms that the robot was indeed facing the pole; at the final pose the pole lay a median of $3.9^{\circ}$ off the robot's heading using sonar (at most $16.4^{\circ}$) and $2.2^{\circ}$ using vision (at most $7.4^{\circ}$).


The one run that did not arrive (pole 2, start 3, sonar) ran out of steps rather than going astray. It ended on its $200$th step in the middle of an approach, having closed the true pole range from $577$ to $507$ to $437$\,mm over its last three steps, against the $400$\,mm at which we would have scored an arrival. What cost it the step budget was not that the model failed to recognize a pole in view, but that the pole was rarely the nearest object in the robot's field: over the $200$ steps the pole was the nearest reflector within $1200$\,mm on only eight of them, and the inverse model called a pole on three of those eight. We report this run as a failure and retain it in every figure and median below. Taken together, these results show that the inverse model, noisy as it is, was sufficient for the robot to complete the task from almost every start pose and for both pole placements.


While both sonar and vision allowed the robot to find and approach the pole, as expected, the modalities resulted in very different approach behavior (Table~\ref{tab:direct-results} and Fig.~\ref{fig:direct-results}). Using sonar, the robot took a median of $52$ steps ($12$ to $200$) and traveled $6.65$\,m ($0.83$ to $29.32$), compared to $34$ steps ($13$ to $91$) and $2.64$\,m ($0.76$ to $11.60$) using vision. The ten start and placement combinations differ widely in difficulty, so the paired comparison is the more informative one: each sonar run shares its start pose, its pole placement and its controller seed with a vision run. Sonar traveled further in all ten pairs and took more steps in nine of them, by a median factor of $1.75$ in path length (range $1.04$ to $5.03$). This difference mainly stems from the limited range over which sonar (and the inverse model) can detect the pole. The sonar runs spent a median of $26$ steps before detecting the pole and an additional $13$ approaching. In contrast, the vision runs needed $4$ steps before seeing the pole and used $25$ approaching. Indeed, vision detected the poles from much farther away than sonar. The robot was a median of $1068$\,mm away from the pole when sonar first perceived it, compared to $2038$\,mm using vision. The sonar figure is also far more tightly bunched, between $1018$ and $1140$\,mm across runs, because it is set by the controller rather than by the arena: a pole call is refused whenever the class-agnostic range head places the nearest reflector beyond $1200$\,mm. Vision locates the pole from most of the arena, while with sonar, the robot must wander until the pole happens to come inside that band. Consequently, the sonar feature reported a pole on $127$ of the $672$ sonar steps, whereas the vision feature reported one on $234$ of $410$ (Table~\ref{tab:direct-results}). Neither modality ever returned an empty feature: the deployed inverse model has an abstain class that it was never trained to use (Table~\ref{tab:inverse-params}), and vision reads the arena geometry directly, so in a closed arena there is always something in the cone for either to report.
...
The model is the same one, so the difference lies in the echoes it was asked about, and two things separate them. First, we used a simpler arena than those used during acquisition: the Experiment 1 arena held a single pole and no interior structure, whereas we collected the training echoes in arenas holding four or five poles. Second, the runs did not sample the pole from arbitrary directions. The controller turns toward whatever it perceives, so a pole seen once tends to sit near the center of the field on the following step. Over the run steps at which a pole was the nearest reflector within $1200$\,mm, the pole lay a median of $4.8^{\circ}$ off the robot's heading and within $15^{\circ}$ on $80\%$ of them; over the corresponding training echoes the median is $17.0^{\circ}$ and the figure is $44\%$. We do not attribute the difference in accuracy to either factor on its own. The point is that the echoes a closed-loop run generates are not a random draw from the distribution the model was evaluated on, and its perception during a run should not be expected to match that evaluation in either direction.  
...
\begin{figure}[!htbp]
    ...
    \caption{Trajectories of the $20$ Experiment 1 runs. Columns give the modality (sonar, vision) and rows the pole placement. Each panel shows the five runs from the five start poses, one color per start and the same colors in every panel, so that a start can be followed across conditions. Triangles mark the start pose and point along the robot's initial heading, stars mark the pose at which the robot arrived, and the cross in C marks the one run that exhausted its $200$-step budget instead. Small filled dots mark every pose at which the modality reported a pole, and open circles every pose at which a pole was the nearest reflector within $1200$\,mm, the range inside which the controller will accept a pole call, but went unreported. The pole is the red disc, and the dashed ring is the $400$\,mm perceived range at which the approach stops. Using sonar, the robot wanders until the pole enters that $1200$\,mm band, so that its detections form a short tail running into the pole (A, C). Using vision, the pole is picked up within a few steps of the start and reported for most of the path (B, D). The missed poles sit at the outer edge of the band, $12$ of the $16$ lying beyond $800$\,mm from the pole; vision has none, since it reads the geometry the tracker also uses. The eight isolated dots in C are the spurious pole calls of the experiment. In every one of them the nearest reflector was in fact a wall, between $1.1$ and $1.4$\,m away, which is the range at which the class head begins to fail (Fig.~\ref{fig:inverse-results}A). The range veto does not catch these, because it tests the range to the nearest reflector whatever its class, and that range lay inside $1200$\,mm. Note that pole 2 lies close to the arena wall, so that it falls outside the forward cone from several headings even when using vision (D).}
