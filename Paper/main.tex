...
The mapping from sensory data to world feature correlates is often called a sensory modality's inverse model \citep{Jordan1992,Wolpert1998,Sutton1990,Ha2018, Rao1999,Friston2010}. Neural pathways implement such inverses to transform sensation into perception and representation of the environment. Sensory modalities differ greatly in what they allow an animal to perceive about its environment \citep[e.g.,][]{Boonman2013,Page2021}. However, modalities also partially overlap in their environmental access, enabling animals to recover features of the world through multiple sensory systems. Bottlenose dolphins, for example, recognize object features across echolocation and vision \citep{Herman1998,Harley2003,Bruck2022}. Rodents access spatial and shape features of nearby objects through both whiskers and vision \citep{Winters2010,Nikbakht2018,Guyoton2025}. 

Even when modalities access the same environmental features, they can differ substantially in how readily they can recover those features. Vision's retinotopic organization, for instance, makes object azimuth directly accessible from activity location in the visual pathway \citep{Wandell2007,Szeliski2022}. For hearing, by contrast, extracting a sound source's location from acoustics is less straightforward and must be learned: extracting source azimuth from the binaural signal requires neural circuitry that compares interaural time and level differences \citep{Grothe2003,Hofman1998}.

The overlap in feature encoding, coupled with asymmetry in ease of recovery across modalities, presents a learning opportunity. If one modality already extracts a given feature, it can supply training labels for another modality that carries the same feature but does not yet extract and represent it. In other words, a modality that recovers a feature can supervise another in learning an inverse model for that same feature, a process we term \cmt. Barn owls are a well-known example of vision supporting the learning of an inverse model for audition. Vision calibrates their auditory space map, and prism-induced visual displacements shift the auditory map accordingly \citep{Knudsen1995}. In this example, the auditory pathway carries spatial information (i.e., the azimuth and elevation of a sound source), but the inverse model is missing. Vision supplies the supervisory signal that labels auditory patterns with world locations, allowing the auditory inverse model to be learned. Once trained, the auditory pathway can run the inverse model on its own, mapping sound to space, and the owl can hunt in darkness. The same kind of cross-modal inverse training is documented in mammals. The auditory space map in the ferret superior colliculus develops normally only with matched visual experience \citep{King1995}. Adult primate and human auditory localization can be recalibrated by sustained audiovisual spatial conflict \citep{Recanzone1998,Hofman1998}.
...
In the example cited above, the supervisor modality (vision) provides a correspondence between two spatial maps of the same variables (azimuth and elevation), allowing the receiver's (hearing) map to be brought into alignment with the supervisor's. However, in principle, the same cross-modal inverse training could be used to learn to extract more complex environmental features, such as object properties (e.g., shape) or affordances (e.g. passibility). This is especially true when an animal has two informationally rich senses that differ greatly in how easily they can extract given environmental features, such as echolocating bats, which use both sophisticated bio-sonar and competent vision. 

Bats are highly maneuverable animals that can fly and forage along established routes in highly cluttered space \citep{Barchi2013,Petrites2009} and have excellent spatial memory \citep{Barchi2013,Holland2007,Geva-Sagiv2015,Moehres1966,VonHelversen2005}. They can navigate to salient locations like roosts, foraging grounds, and water sources \citep{Schnitzler2003,Helversen2003}. They can navigate over kilometers using echolocation alone \citep{Goldshtein2024,Stones1969,Williams1966}. Therefore, echoes must carry the information these behaviors require. That information is nonetheless hard to extract \citep[e.g.,][]{Vanderelst2015,Vanderelst2016,Steckel2013,Mansour2019,Mansour2019a}. Sonar collapses the 3D world into just two (slightly different) 1D signals, one per ear. This collapse preserves the delay of the first echo and the distance to the nearest reflector, but little else in a readily usable form.  Most natural objects, vegetation in particular, return a multitude of overlapping and interfering echoes \citep{Yovel2009,Yovel2011,Vanderelst2016,Warnecke2018}. Echoes arriving within about 200 to 400\,$\mu$s of each other merge in the auditory periphery \citep{Simmons1989,Wiegrebe1996}, so that reflectors separated by less than three to seven centimeters in range are not resolved, and neither are objects less than about 40$^{\circ}$ apart \citep{Geberl2019}. Sonar, in short, does not hand the bat an acoustic image. Environmental features such as the shape, size, and orientation of objects, if encoded, have to be extracted from blended waveforms, whereas vision has direct access to many of them.

Echolocating bats also see. The peak density of retinal ganglion cells sets an upper bound on spatial resolving power, which for \animal{md}, an aerial hawking insectivore, is about 0.6 cycles per degree \citep{Cechetto2020}. Behaviorally, optomotor experiments give values of the same order \citep{Bell1986}.
Gleaning and carnivorous species, which have substantially larger eyes, reach values several times higher \citep{Pettigrew1988,Ekloef2014,Thiagavel2018}. To contextualize, bats' resolution is comparable to that of similarly sized rodents \citep{Veilleux2014,DeSousa2022} and roughly fifty times coarser than normal human vision, which achieves about 30 cycles per degree \citep{DeSousa2022}.

While bat vision has low resolution, it is highly sensitive. The retina of echolocating bats is dominated by rods \citep{Suthers1970}, the photoreceptor class that subserves vision in dim light in all vertebrates, and recordings in the superior colliculus of \animal{pd} find unusually large visual receptive fields, which suggests that these bats improve dim-light performance by spatial rather than temporal summation \citep{Hoffmann2019}, at the cost of resolution. \citet{Cechetto2023} determined the visual detection threshold of \animal{md} behaviorally in a two-alternative forced choice task and found a luminance threshold of $3.2(\pm0.9)\times10^{-4}$ cd m$^{-2}$, which corresponds to the luminance of terrestrial objects in open habitat under a clear, moonless, starlit sky. Earlier measurements are consistent with this figure: optomotor responses persist down to about $6\times10^{-4}$ cd m$^{-2}$ \citep{Bell1986}, brightness discrimination in \animal{ef} down to about $10^{-4}$ cd m$^{-2}$ \citep{Ellins1974}, and flash-evoked potentials give values of the same order in an emballonurid \citep{Liu2015}, though electrophysiological estimates typically yield higher thresholds than behavioral ones and should be treated as conservative. A comparably low threshold has been reported for the nectar-feeding \animal{gs} \citep{Winter2003}. 

The low resolution of bat vision is unlikely to support the perception and pursuit of small insect prey \citep{Rydell2003}. However, low resolution but high sensitivity are sufficient for vision-based spatial orientation \citep{Cechetto2023,Boonman2013} and for visually perceiving the local layout of the environment, i.e., the layout of the vegetation and other objects in the scene. Even at the resolution of 0.6 cycles per degree estimated for \animal{md} \citep{Cechetto2020}, an aerial hawking insectivore would resolve an object 2 cm across out to about 1.4 m, and \citet{Boonman2013} estimated that its sensitivity allows it to detect the same object out to 4 m at 5 lux. The same object will return a perceivable echo as well \cite[e.g.,][]{Stilz2012}. Indeed, while, as mentioned, bats can navigate and forage using sonar alone, both senses are available at the same time over a substantial part of the bat's activity period. Some species echolocate in full daylight \citep{Eitan2022} and many species emerge at dusk, under intermediate light levels of roughly one to ten lux, and continue to echolocate throughout \citep{Boonman2013,Holland2005}. For example, \animal{md} begins foraging about thirty minutes after sunset and remains active for one to two hours \citep{Rydell1996}.

%Bats have binocular vision, although it has been characterized in only a few species. \citet{Bell1986} found that \animal{mc} had roughly twice the binocular overlap of the other species they examined, and \citet{Hoffmann2016} measured the visual field of \animal{pd} directly, finding a maximum binocular overlap of 40\dg{} that spans nearly 90\dg{} in elevation and is widest between 25 and 50\dg{} above the horizon, so that in level flight the binocular field is directed slightly upward. The monocular field extends 135\dg{} and is not obstructed by the noseleaf. Importantly, the area centralis, where ganglion cell density is highest, subserves this binocular field, so that the bat sees best where the two eyes overlap. Binocular disparity over that region, together with the parallax generated by the animal's own flight, can in principle supply distance as well as direction, and across the whole field of view rather than only at the nearest reflector, although stereopsis has not been demonstrated behaviorally in any bat. The biosonar beam of \animal{pd} is narrow and directed frontally and slightly below the horizon \citep{Vanderelst2010}. In contrast, the visual field is wider and tilted slightly above the horizon, so vision monitors a considerably larger volume of space than a single call can probe \citep{Hoffmann2016}.

The concurrent use of sonar and vision provides bats with an interesting opportunity for \cmt. Both senses encode information about space and objects, but with markedly different ease of access \citep[e.g.,][]{Holland2005,Page2021}. Vision can provide supervisory labels to train sonar's inverse model, allowing bats to unlock their auditory system's potential without solving the full inference problem from scratch.

As stated in the dicussion, direct evidence that bats perform \cmt\ remains limited, mostly due to a lack of experiments set up specifically to test this ability. Here, we use a synthetic approach to demonstrate the feasibility of \cmt\ in a bat-like robot as a proof of principle for how bats might exploit vision to train sonar-based feature extraction. We trained an inverse model to recover, from a single binaural echo, the class of the nearest reflector and its local geometry, using labels derived from a proxy for vision. The resulting inverse model is noisy and limited in range. However, the main question is not how accurate the inverse model is, but whether it is sufficient to support behavior. We tested this in two tasks: approaching an object while avoiding walls, and holding a learned route through an arena. 

\section{Methods}

%We used a small mobile robot equipped with a bat-like sonar system, to show that acquiring an inverse model and using it to control behavior is biologically plausible. The robotic model operates in environments that return real, complex echoes. Moreover, as detailed below, the robot's sonar is more limited than bat biosonar: we use a narrowband emitter (instead of the broadband emissions typical of bats in dense environments), long pulses (instead of the brief pulses bats use under such conditions), and a low carrier frequency (instead of the higher frequencies bats typically use). Vision, on the  

%The robot's teacher, in contrast, is better than a bat's. Its visual labels come from the digitized geometry of the arena, are restricted to the robot's forward field of view, and therefore are free of the noise and limited resolution of bat vision. Our results thus establish that even a severely limited sonar can be taught an inverse model that supports behavior, given a reliable teacher. How closely bat vision approaches such a teacher is a question the acuity and sensitivity data above speak to but do not settle.
...
\begin{figure*}[htb]
    ...
    \caption{Robot and arena setup. Left: Close-up of the robot showing the 3 MaxBotix ultrasonic transducers. The blue plastic scale indicator measures 5 by 5 cm. Middle: A view of the arena with the robot. The yardstick in the arena is about 90 cm long. Right: An example of the sonar data as acquired by the two `ears' of the robot.}
    \label{fig_robot}
\end{figure*}
...
We used a Pololu 3pi+~2040 robot (Fig.\ref{fig_robot}) (Pololu Robotics \& Electronics, Las Vegas, NV): a commercially available differential-drive platform with an RP2040 microcontroller (diameter: 96 mm). We added a HiLetgo ESP8266 ES Wi-Fi module to run all higher-level control logic off-board on a host computer, while the robot executed low-level motion and acoustic commands on request. The robot communicated with the host via JSON-framed messages over a TCP link. The drive was not perfectly straight: driving forward also turned the robot slightly, by an amount unrelated to any commanded rotation, mostly through asymmetric wheel slip. We measured this curl before each session and pre-compensated every drive command for it, adding the opposite rotation so that the net heading change matched what the controller had asked for. The compensation was applied below the controller, which was told neither the correction nor what remained of the fault after it.

We mounted three MaxBotix MB1360 XL-MaxSonar-AEL0 ultrasonic transducers (MaxBotix Inc., Brainerd, MN) on the robot in a 3D-printed housing. Two transducers acted as left and right ``ears'': their centers sat 37 mm apart and each was angled outward by 15$^{\circ}$ in azimuth, so that their main axes diverged by 30$^{\circ}$. The third transducer was mounted on the midline, 15 mm below the ears, and pointed straight ahead; it acted as the emitter, or ``mouth''. Emissions were tone bursts at a carrier frequency of 42 kHz and a duration of approximately 2.5 ms. Each ear recorded the resulting echoes as the demodulated envelope, sampled by the robot's analogue-to-digital converters at 10 kHz for 200 samples per ping, i.e., a 20 ms acquisition window per ear. An example measurement is plotted in Fig.\ref{fig_robot}. It should be noted the narrowband sonar system of the robot is limited with respect to that of bats. Indeed, especially in cluttered conditions, (FM) bats typically use very broadband calls \cite[see][for references]{Falk2014}. For example, \animal{ma} used calls with a bandwidth of 50 kHz (duration: 3.6 ms) in open space while using calls with a bandwidth of 70 kHz (duration: 1.3 ms) ins  4.4 by 3.5 m flight room.
...
In the second experiment, we asked whether the inverse model can support a more complex task. We selected a task inspired by the stable flight paths observed in bats, both in experiments \citep{Barchi2013} and in the field \citep{Holderied2006}. In particular, we modeled a bat that has learned to fly a stable route using features that vision makes readily available and for which it has an inverse model. In other words, it follows the path using features it can also retrieve from sonar. This enables the bat to fly the same route supported by sonar alone, if those features can be extracted with sufficient fidelity. Currently, it is unknown whether bats can transfer route following learned through vision (alone) to sonar. Although this could be tested under experimental conditions, bats in the wild would always have access to sonar. However, for this paper, i.e., demonstrating that a noisy inverse model supports non-trivial behavior, this hypothetical scenario is a good test case.

To determine the path the robot should follow, we manually drew a closed loop over the digitized arena, along with a rectangular release region and heading, and resampled it at $25$\,mm spacing so any position could be expressed as a distance traveled along the loop. We implemented the controller as a recurrent network with $32$ hidden units. At each step, it received the same local feature the inverse model supplies in Experiment 1, together with the rotation it commanded in the previous step, and returned a rotation, capped at $90^{\circ}$ (Fig.~\ref{fig:control-loop}).

\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{fig_control_loop}
    \caption{The control loop deployed in Experiment 2. Sonar returns one echo envelope per ear, and the inverse model turns these into the features listed beneath the arrow. Those features are one of the network's two inputs. The other is the motor command the network itself produced on the previous step --- the same signal that goes to the motors, which is why the two arrows carry the same label. Nothing else reaches the network. The motors execute the command with error, so the pose the robot arrives at is not the one it commanded, and no sensor reports the difference: there is no odometry and no inertial sensor. The only route by which that discrepancy can reach the network is the next emission, made from wherever the robot actually ended up (dashed). The network therefore never learns what the robot actually did, only what it asked for.}
    \label{fig:control-loop}
\end{figure*}

In simulation, we trained a controller to keep the robot on the path. We did not model how the route was chosen, or where the corrections that keep the robot on it (while using vision) originate. In other words, we assumed that the bat already knew how to follow the path visually. We only modeled acquiring a controller that could mimic the path using sonar, using the features extractable by the inverse model. A pure-pursuit controller with access to the robot's true position drove the route, projecting that position onto the loop, advancing $240$\,mm along it, and returning the rotation that pointed at the resulting target. We trained the controller with no access to the robot's position. It received only the local feature and the rotation it had commanded on the previous step, so that it could learn only what features recoverable from sonar can support. The features presented during training carried realistic error, drawn from the errors the inverse model makes on real echoes.

We applied two kinds of perturbation during training. First, on $30\%$ of steps we disturbed the commanded rotation by about $30^{\circ}$ while keeping the clean rotation as the training target, so that the controller also learned to return to the path after leaving it. Second, because the real robot does not execute motor commands exactly, neither did the simulated motors. Besides a small per-step error, we gave each episode two sustained faults of the kind the real robot shows, each drawn once at the start of an episode and held for its whole duration: a gain error of up to $15\%$ in rotation and $5\%$ in distance, and an additive rotation bias of up to $5^{\circ}$, applied at every step. The additive term models drive curl, the robot's dominant fault: it turns slightly while driving forward by an amount unrelated to the commanded rotation, so a multiplicative gain error cannot reproduce it. Neither fault averages out over a lap.

We then deployed the trained controller on the robot, released it from the release region, and let it run for several laps. At each step, the robot executed the rotation the controller generated (in open loop, so the execution was susceptible to noise), after which the robot drove forward $150$\,mm. We first ran the arena as we had trained the controller, and repeated that run. The difference between these two runs sets a standard against which the effect of any manipulation can be judged. 

In principle, since the forward movement between rotations is fixed, the robot could keep track of the number of steps executed and associate a distance or time point along the track with the motor commands to keep the robot on the track. In other words, the robot could use a form of dead reckoning to complete the path. However, the controller was trained in a way that prevented dead reckoning alone. As mentioned, we injected noise into the training by adding it to the motor commands generated by the controller during execution. Hence, the controller had to learn to keep a noisy plant on the path, and dead reckoning would be insufficient to keep the simulated robot on the path during training. This noise was injected precisely because the physical robot also experiences noise. As a result, the training forced the controller to use sensory input to correct errors. To demonstrate that the robot indeed relied on sensory input in addition to dead reckoning to follow the path, we then ran conditions in which we removed or displaced objects (see the results section for details of the manipulations). If what the robot senses does not matter, the route should be unchanged; if it uses what it senses to stay on track, the route should change. We carried out the whole procedure in two arenas, each with its own path and its own controller.
