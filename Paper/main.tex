...
Fig.~\ref{fig:inverse-results}B shows the error of the models in predicting a pole’s azimuth from the echoes, again as a function of distance. We compare the models’ performance with a baseline for a model that returns 0 degrees, irrespective of the input. This plot shows that the quadrant models stop outperforming the baseline at around $2000$\,mm. The deployed model still seemed to outperform the baseline beyond this distance. However, that comparison is against a baseline computed over all echoes, rather than over the $23$ echoes beyond $2000$\,mm on which the deployed model could be scored. Measured on those same echoes, the baseline error is $4.9$ degrees and the deployed model achieves $6.8$: it does not outperform the baseline either. For this reason we show only the quadrant models in Fig.~\ref{fig:inverse-results}B. 
...
Fig.~\ref{fig:inverse-results}F asks whether the classifier's confidence can be believed. The classifier does not emit a bare label: it emits a probability for each class, and panel A reports how often the most likely one was correct. Panel F asks what the value of that probability is worth, comparing the probability the model reported with how often it turned out to be right when reporting it. Points on the diagonal mean the reported probability can be taken at face value. Our model stays close to the diagonal: when it reports being $90\%$ certain, it is correct on $97.8\%$ of those echoes. This is useful for a controller, which can act on the class where the model is confident and disregard it where it is not, without needing a separate category for \emph{unsure}.
