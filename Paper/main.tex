...
In bats, the evidence is indirect but points the same way. In the omnivorous bat \animal{pd}, which has large eyes and orients by both vision and echolocation, the deep layers of the superior colliculus hold a topographic map of sound azimuth that is in register with the visual map in the layers above \citep{Hoffmann2016}. No such map has been found in insectivorous species with poorly developed vision \citep{Allen2021}, and \citet{Hoffmann2016} suggest that in these species vision supplies too little input to instruct one. A map that has not been found may still exist. It is also unclear, however, what a map of echo direction would represent for a bat. A spatial map assigns each sound a single direction, which suits an external source or an isolated reflector such as a flying insect. Most of the echoes a bat receives are instead blends of reflections from extended objects (see below), and have no single direction to map. Note that most of these examples share a common form: the supervisor modality provides a correspondence between two spatial maps of the same variables (azimuth and elevation), allowing the receiver's map to be brought into register with the supervisor's. The inverse we consider below differs fundamentally. It is object and feature based rather than map-based: it must first determine what kind of reflector the sensory data came from and only then recover the geometry appropriate to that kind. Learning an inverse of this sort is therefore likely to be more demanding than aligning two maps of the same space. The brain appears to treat the two problems separately as well: in humans, binding an object's features across modalities depends on perirhinal cortex, whereas the posterior superior temporal sulcus integrates cross-modal perceptual features irrespective of their meaning \citep{Taylor2006}.

Echolocation is informationally rich. Bats fly stable routes through cluttered space, also in complete darkness \citep{Barchi2013,Holderied2006}, and can navigate over kilometers using echolocation alone \citep{Goldshtein2024}, so the echoes must carry the information these behaviors require. That information is nonetheless hard to extract. Sonar collapses the 3D world into just two signals, one per ear. This collapse preserves the delay of the first echo, and with it the distance to the nearest reflector, but little else in a readily usable form. Bats localize a single isolated target with high accuracy, but this does not imply that they can recover the layout of many reflectors at once. Most natural objects, vegetation in particular, return a multitude of overlapping and interfering echoes \citep{Yovel2009,Yovel2011,Vanderelst2016,Warnecke2018}. Echoes arriving within about 200 to 400\,$\mu$s of each other merge in the auditory periphery \citep{Simmons1989,Wiegrebe1996}, so that reflectors separated by less than three to seven centimeters in range are not resolved, and neither are objects less than about 40$^{\circ}$ apart \citep{Geberl2019}. Sonar, in short, does not hand the bat an acoustic image. Features such as the shape, size, and orientation of objects have to be extracted from blended waveforms, whereas vision has direct access to many of them.

Echolocating bats also see. Their retinas are dominated by rods \citep{Suthers1970}, and their vision trades resolution for sensitivity \citep{Hoffmann2019}. Retinal ganglion cell density limits the aerial-hawking insectivore \animal{md} to about 0.6 cycles per degree \citep{Cechetto2020}, and optomotor responses place \animal{ef} at a similar value, with gleaning species resolving up to an order of magnitude finer \citep{Bell1986}. This is comparable to similarly sized rodents and some fifty times coarser than human vision \citep{DeSousa2022}. Sensitivity, in contrast, is high: \animal{md} detects visual stimuli at luminances corresponding to open ground under a moonless, starlit sky \citep{Cechetto2023}.

Vision of this grade cannot support the pursuit of small insects \citep{Rydell2003}, but it suffices to see the layout of the surroundings \citep{Boonman2013,Cechetto2023}. At 0.6 cycles per degree, an object 25\,mm across, the diameter of the poles used below, is resolved out to about 1.7\,m, and extended surfaces from much farther. Vision need not supply range, which echo delay provides directly; what it can supply is what kind of reflector lies in which direction. Both senses are, moreover, available together. Many species emerge at dusk, at one to ten lux, and echolocate throughout \citep{Boonman2013,Rydell1996}, before fading light leaves them to sonar alone \citep{Cechetto2023}. Where the two senses conflict, bats follow vision, colliding with obstacles their sonar should have revealed \citep{Eklof2002,Orbach2010}.
