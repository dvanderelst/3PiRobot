...
We deployed one controller in each of the two arenas. Each was trained in simulation on the path drawn for its own arena (Section~\ref{sec:inverse-training}), and each was then released from its release box and left to run. Every run lasted $500$ sense-act steps, which is several laps of either path. The controller was never retrained between conditions: within an arena, every run below uses the same network, so that what differs across conditions is the arena and not the robot.

For every run we logged the trajectory from the overhead camera system. We report the number of laps the robot completed, its distance from the trained path at each step, and, in the conditions where we displaced an object, how far the route moved toward that object's new position.

An effect of a manipulation has to be judged against something. Repeating a run does not reproduce it exactly: the robot starts from a slightly different pose, its motors do not execute a command identically twice, and the sonar returns a different sample of echoes. We therefore ran the unmanipulated arena twice before manipulating anything, and treat the difference between those two runs as the floor that any effect has to clear. In both arenas that floor turned out to be small, so the comparisons below are between an effect and a repeat rather than between an effect and zero.

We describe the two arenas in turn, and then what the two together support.
