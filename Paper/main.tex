...
Fig.~\ref{fig:inverse-results}B shows the error of the models in predicting a pole’s azimuth from the echoes, again as a function of distance. We compare the models’ performance with a baseline for a model that returns 0 degrees, irrespective of the input. This plot shows that the quadrant models stop outperforming the baseline at around $2000$\,mm. The deployed model still seemed to outperform the baseline beyond this distance. However, this is an artifact of which echoes it was scored on. Running the quadrant models on the deployed model's own test echoes gave very similar performance ($8.1$ against $7.8$ degrees of median error). Those echoes are simply easier: on the echoes the deployed model did not hold out, the quadrant models score $10.1$ degrees. Hence, the apparent advantage of the deployed model at larger distances is a property of its test set rather than of the model. 
