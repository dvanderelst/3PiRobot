...
To determine the path the robot should follow,                we drew a closed loop over the digitized arena, along with a rectangular release box and a release heading, and resampled it at $25$\,mm spacing so any position could be expressed as a distance traveled along the loop.                     We implemented the controller as a recurrent network with $32$ hidden units.                     At each step,             it received the same local feature the inverse model supplies in Experiment 1,  together with the rotation it commanded in the previous step, and returned a rotation, capped at $90^{\circ}$, after which the robot drove forward $150$\,mm. Because the network is recurrent, it can combine what it senses with a record of past rotations and sensory data.
...
We then deployed the trained controller on the robot, released it from the start region, and let it run for several laps. We first ran the arena as we had trained the controller, and repeated that run. The difference between these two runs sets a standard against which the effect of any manipulation can be judged. To test whether the robot used sensory input from the poles and walls, rather than reproducing the route solely from its own movements (i.e., using only path integration), we then ran conditions in which we removed or displaced objects (see results section for details about the manipulations performed). If what the robot senses does not matter, the route should be unchanged. If the robot used sensory input to stay on track, the route should change. We used this approach because it is impossible to simply deprive or clip sensing data. The controller combines sensing data and ego-motion in a non-separable way, and altering the sensory data would also inhibit the robot's ability to use path integration.

For every run, we logged the robot's trajectory from the overhead camera system. We report the number of laps completed, the distance from the path at each step, and, in the displaced-wall condition, how far the trajectory shifted toward the new wall position. 


\dnote[18]{Only one item of this note is still live: the biological framing for this experiment is not written. That is landmarks against path integration, and the Neuweiler and M\"ohres (1967) precedent in scratchpad section 6. Everything else it carried is discharged, checked against the two deployed policies' configs on 2026-08-21: the controller reads the same inverse as Experiment 1 with poles included (\texttt{use\_poles}), the $2$\,m clamp became $2500$\,mm, the step is $150$\,mm with the teacher lookahead scaled to $240$\,mm to match, and both arenas have been built, trained and run. Two of its own details were wrong by the end: it listed the $\sigma$ channels as part of the observation, but $\sigma$ is deliberately off (\texttt{use\_sigma=False}, since the simulator emits a per-bin $\sigma$ that would identify the true distance bin), and its echo count and pole-recall bands predate the six-session retrain.}
