...
We trained each controller in simulation on the path drawn for its arena (Section~\ref{sec:inverse-training}) before deploying them in the arenas, starting from the release box. Every run lasted for $500$ sense-act steps, allowing the robot to complete several loops of the path. For each path, we completed two runs using the arena layout the controller was trained on. Completing two runs allowed us to assess baseline variation in the robot's behavior, which is not zero: the robot starts from a slightly different pose each time and its motors do not execute a command identically twice. In addition, we completed runs with changes to the arenas, that is, the removal or displacement of poles, blocks or a wall. The controller was not retrained for these runs.
