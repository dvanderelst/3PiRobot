...
\begin{figure}[!htbp]
    ...
    \caption{Arena 2: the trained route and what each change to the arena did to it, drawn as in Fig.~\ref{fig:arena1-runs}. Blue marks the digitised layout, purple the poles, the dashed grey line the trained path and red the flown trajectory. (A) The arena as trained, run twice. (B) The two northern poles removed, crossed where they stood. (C) The same two poles displaced, run twice; the circles mark which poles moved. (D) A section of the southern wall displaced inward by about $450$\,mm, marked in black with an arrow giving the direction; its new position was not digitised, so the arrow is schematic. (E) Distance from the path against position around the loop, as a median in a sliding window of $500$\,mm of path, with the stretch the displaced wall borders shaded and the manipulated objects marked. Removing the poles raises the profile where they stood, displacing them changes nothing anywhere, and displacing the wall produces a single tall peak where it borders the loop.}
