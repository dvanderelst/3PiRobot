...
We trained the inverse model on data from all six sessions, and evaluated it on two different spatial splits, which serve different purposes.

The first split produced the network we deployed. From each session we held out $15\%$ of the echoes as a single contiguous patch, those collected nearest a randomly chosen location, trained on the remaining $85\%$, and used the patch for early stopping. Because a patch is a coherent region of the arena rather than a scatter of poses, it consists of locations the model never trained on, so the gap between training and held-out performance is an overfitting check (Table~\ref{tab:inverse-results}). Taken alone it is a noisy estimate, resting on the $418$ echoes of the patches, of which $175$ are poles.

The second split is a four-fold cross-validation, and it is what the Results report. We divided each session's recording positions into four spatial quadrants and held out one quadrant in turn, training four models. Every echo is then held out exactly once, and can be scored by the one model that never saw it, which gives a held-out estimate across all $2775$ echoes rather than across a single patch. This matters for any result that varies with range, where one patch leaves too few echoes per range band to resolve. Since the quadrants are contiguous regions, these folds also measure how the model behaves at positions it was not trained on.

The four folds characterize the training procedure rather than the particular network we deployed, which is a separate draw from that procedure. We therefore report the deployed model against the folds (Fig.~\ref{fig:inverse-results}), so that its behavior can be seen to match rather than assumed to. Beyond this we did not use a separate validation set; we consider successful task completion to be the validation of the deployed model.
...
We trained 5 different versions of the inverse model. The first four trained with one arena quadrant held out in turn (Section~\ref{sec:inverse-training}).
