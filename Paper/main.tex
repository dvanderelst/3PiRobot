...
The two arenas differ, however, in what displacing an object did. In Arena 1, moving the two blocks carried the route with them, by about a third of their displacement, on every lap and only over the stretch they border. Those blocks left the path clear once moved, standing $461$ and $170$\,mm from it, so the robot was not steering around them; the route moved because the geometry it was keying on had moved. In Arena 2, displacing the poles by comparable and larger distances changed nothing at all. For the poles, then, presence mattered and position did not. This follows from what the inverse model can recover: blocks are extended surfaces and enter the wall-distance profile directly, whereas a pole must first be classified as one, and that classification is no better than baseline beyond about $1500$\,mm, with pole range trained only within $1$\,m. The controller therefore has a positional signal from extended surfaces and, at best, a presence signal from poles. The controller has no odometry and no position sense, but it does not need one to keep a rough account of its own progress: it drives a fixed $150$\,mm at every step, so counting steps is itself a measure of distance traveled, and the recurrent state can hold such a count. What these manipulations show is that the route does not rest on that account alone. A controller replaying a remembered sequence would have been untouched by them.

These manipulations concern what the robot does with what it hears at a given place on the route. The runs settle a further question about the route as a whole. Driving forward also turned the robot, by between $1.9$ and $6.1^{\circ}$ per step across the eleven runs, by an amount unrelated to any commanded rotation. The per-session calibration removed most of this but not all: what remained ranged from $-1.2$ to $+3.0^{\circ}$ per step, and the motors executed as little as $90\%$ of the rotation they were given. At the largest residual, a single lap accumulated some $237^{\circ}$ of heading that nothing in the controller's input reported, since it is told the rotation it requested and neither the correction applied below it nor the error left in that correction. A controller counting its own commands would have left the route within a lap. None of these runs did, and the largest residual of the whole experiment belongs to one of the pole-displacement runs, which tracked the path as closely as the two unmanipulated ones. That also disposes of the reverse concern: the run carrying the most residual curl produced no route change at all.
