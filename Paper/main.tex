...
Fig.~\ref{fig:inverse-results}B shows the error of the models in predicting a pole’s azimuth from the echoes, again as a function of distance. We compare the models’ performance with a baseline for a model that returns 0 degrees, irrespective of the input. This plot shows that the quadrant models stop outperforming the baseline at around $2000$\,mm. The deployed model still seemed to outperform the baseline beyond this distance. However, this is not a difference in ability. Running the quadrant models on the deployed model's own test echoes gave the same result ($8.1$ against $7.8$ degrees of median error, a difference well inside sampling noise). At these distances the deployed model's curve rests on only $17$ to $30$ echoes per band, so the apparent advantage should not be read as one. 
