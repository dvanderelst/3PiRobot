...
\dnote[23]{\textbf{Plain-language summary, written as a reading aid rather than as paper prose.} Use it to check the five paragraphs below say what you expect; it can also become the opening paragraph if you want one.
\emph{What the model can do.} Told only the two echo envelopes, it can say how far away the nearest thing in front of the robot is, and it can do that all the way out to about $2.5$\,m, getting it right to roughly ten per cent. Close in, within about a metre, it can also say \emph{what} that thing is, wall or pole, and roughly \emph{where} a pole is to left or right. It knows how confident it should be: when it says it is $90\%$ sure, it is right about $98\%$ of the time.
\emph{What it cannot do.} It cannot tell a wall from a pole beyond about $1.5$\,m. Past that it is not merely uncertain but wrong more often than a rule that ignores the echo and always answers ``wall''. Pole bearing lasts somewhat longer, to about $2$\,m, and then also fails. And the head that measures the distance \emph{to the pole specifically} stops working past a metre by construction, because it was only ever trained inside a metre.
\emph{The one sentence that matters.} Far away, the model still knows how far something is; it just does not know what it is. Measuring is easy, naming is hard, and the two fail at different distances.}

Unless stated otherwise, the figures below are out-of-fold: each of the $2775$ echoes was scored by whichever of four models had not been trained on it, those four having been trained with one arena quadrant held out in turn (Section~\ref{sec:inverse-training}). Because the quadrants partition the data, every echo is held out exactly once, which gives a held-out estimate over the whole set rather than over a single slice of it. Overall, the class of the nearest reflector was recovered on $79.4\%$ of echoes.

Those four models are not the model we deployed, which was trained separately on $85\%$ of a single spatial split, so the figures above describe the training procedure rather than the particular network the robot carried. The two agree. Scored on its own held-out $15\%$, the deployed model recovers the class on $79.2\%$ of $418$ echoes, against the $79.4\%$ obtained across the folds, and it follows the same curves band by band wherever that holdout has the members to show it (open triangles in Fig.~\ref{fig:inverse-results}A--C). The deployed network is therefore an ordinary draw from the procedure rather than a fortunate one. Beyond about $1400$\,mm its holdout becomes too thin to resolve, and the out-of-fold curves carry the description alone.

A single accuracy figure averages over a strong dependence on range, and that dependence is the result (Fig.~\ref{fig:inverse-results}).

The three quantities the inverse reports do not fail together. Classification degrades steadily with the range of the nearest reflector, from $93.2\%$ within $500$\,mm to $71.1\%$ between $1000$ and $1400$\,mm, and crosses the majority-class baseline near $1500$\,mm (Fig.~\ref{fig:inverse-results}A). Beyond $1700$\,mm it is no longer merely uncertain: at $39.6\%$ and $39.1\%$ in the next two bands it is below the $59.4\%$ and $51.8\%$ that a rule ignoring the echo and always answering \emph{wall} would achieve. Pole azimuth survives further. Its median error stays near $9^{\circ}$ against a straight-ahead baseline of $13$ to $18^{\circ}$ out to about $2000$\,mm, and only past that does it stop beating that baseline, reaching $19.3^{\circ}$ against $18.5^{\circ}$ in the last band (Fig.~\ref{fig:inverse-results}B). Range does not fail at all within the span we can measure. This ordering follows the amount of comparison each quantity requires: distance is a time of flight, available from one ear the moment an echo is detected; bearing requires the two ears to be compared; and class requires fine temporal structure to survive that comparison.

The contrast between the two range heads separates sensing from attribution. Both were trained on the same echoes with the same architecture, and they differ only in what they were asked. The head asked \emph{how far is the pole} was trained on pole echoes within $1$\,m; past that it saturates, predicting $768$\,mm on average for the $429$ poles that lie beyond a metre, whatever their true distance. The head asked \emph{how far is the nearest reflector, whatever it is} tracks the truth across the whole span, predicting $450$, $1062$ and $2524$\,mm for true ranges of $200$ to $500$, $1000$ to $1400$ and $2500$ to $3300$\,mm (Fig.~\ref{fig:inverse-results}C), with an error of $13.7\%$ of range in that last band. Crucially it does so equally well for both classes in the region where it cannot tell them apart: beyond $2$\,m its error is $340$\,mm on walls and $303$\,mm on poles, $14.3\%$ and $12.2\%$ of range. The model can locate what it cannot name. The same holds for the wall depth profile, which Experiment 2 steers on: its per-direction error beyond $1400$\,mm ($317$, $226$ and $322$\,mm for left, center and right) is no worse than over the data as a whole ($327$, $326$ and $302$\,mm). What collapses at range is the naming of the reflector, not the measuring of it.

This matters because sonar is often assumed to deliver distance for free. It does so only for an isolated reflector, where one echo can be attributed to one object without ambiguity. In a room the cone almost always holds several reflectors, and the difficulty is deciding which returned echo belongs to what. Our two heads make that visible: asked to attribute a distance to a named object, the network is confined to the range at which it can still do the naming; asked only to report the distance of whatever is nearest, it reaches nearly three times as far.

The classifier's confidence is also usable rather than decorative. Its expected calibration error is $0.020$, so the probability it reports can be read as a probability: thresholded at $0.7$ it is right on $90.7\%$ of the $67.5\%$ of echoes it retains, at $0.8$ on $95.0\%$ of $55.4\%$, and at $0.9$ on $97.8\%$ of $42.9\%$ (Fig.~\ref{fig:inverse-results}D). A controller can therefore act on the class when the inverse is confident and withhold action when it is not, without a separate abstain class and without a range cut, and this is what the terminal approach of Experiment 1 exploits.

Finally, what the model perceives depends on what else is in the beam and not only on the target. Restricting the comparison to a single range band, so that range cannot confound it, echoes whose target was uncontested were classified far better than echoes with a nearer competing reflector outside the analyzed cone.\dnote[24]{Numbers deliberately omitted pending recomputation. The recorded figures, $80.6\%$ against $65.1\%$ at $1000$--$1400$\,mm, come from the 2026-08-12 diagnostic in which the pole-range head was retargeted, not from the deployed two-head model that every other number in this subsection now describes. The analysis needs redoing out-of-fold on \texttt{inverse\_q*} before the numbers go in.} The effect is a property of the scene rather than of the network, and it also bounds what our measurements can show: in an arena of this size an uncontested reflector cannot exist much beyond $1.9$\,m, so the range at which discrimination fails here is a statement about rooms like this one, not about the sensor in free space.
...
\begin{figure}[!htbp]
    ...
    \caption{What the inverse model recovers, against the range of the nearest reflector in the cone. All panels are out-of-fold: each of the $2775$ echoes is scored once, by the fold that did not see it. (A) Classification accuracy, against the majority class of each band. The model crosses that baseline near $1500$\,mm and falls below it beyond. (B) Median absolute pole-azimuth error, against the error of always answering straight ahead. (C) Both range heads. The head trained to report the pole's distance within $1$\,m flattens once past its training domain, while the class-agnostic head follows the diagonal to $2.5$\,m; bars are $\pm$RMSE. (D) Reliability of the class posterior: the probability the model reports against the frequency with which it is right. Open triangles in A--C are the deployed model scored on its own held-out $15\%$, plotted only in bands where that holdout has at least $15$ members; they show the deployed network following the same curves as the folds it was not part of. The dotted vertical line in A--C marks $1400$\,mm.}
