...
To determine the path the robot should follow, we drew a closed loop over the digitized arena, along with a rectangular release box and a release heading, and resampled it at $25$\,mm spacing so any position could be expressed as a distance traveled along the loop. We implemented the controller as a recurrent network with $32$ hidden units. At each step, it received the same local feature the inverse model supplies in Experiment 1, together with the rotation it commanded in the previous step, and returned a rotation, capped at $90^{\circ}$, after which the robot drove forward $150$\,mm. Because the network is recurrent, it can combine what it senses now with what it sensed earlier. The rotation fed back is the one the network commanded, not the one the motors executed: the robot has no odometry, no inertial sensor and no access to its own position. Since those commanded rotations were themselves computed from earlier echoes, the network's state at any step is a function of the echoes it has heard and of nothing else. Any record it keeps of where it is on the route is therefore built from sonar. Fig.~\ref{fig:control-loop} shows the loop this closes.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{fig_control_loop}
    \caption{The control loop deployed in Experiment 2. An emission returns one echo envelope per ear; the inverse model turns these into the feature vector listed beneath the box, and the recurrent network turns that into a commanded rotation, which the motors execute. Two paths lead back to the network, and one of them does not exist. The efference copy (purple) carries the rotation the network itself commanded, and therefore nothing the network had not already computed. The executed rotation (red) does not return at all: the robot has no odometry, no inertial sensor and no sense of its own position, so what the motors actually did is never reported to it. The only route by which the consequences of moving reach the network runs through the environment (brown) --- the robot moves, the geometry around it changes, and the next echo carries the difference. The network's state at any step is therefore a function of the echoes it has heard.}
