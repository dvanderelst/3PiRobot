...
In the results, we compare the performance of the first four models with that of the deployed model. This comparison allowed us to assess whether the performance we obtained for the deployed model is stable or depends on idiosyncratic (spatially coherent) samples in the dataset. If the deployed model's performance matches that of the other four, we can assume it is stable and does not depend on specific training samples. Beyond this, we did not use a separate validation set; we consider successful task completion to be the validation of the deployed model. 
