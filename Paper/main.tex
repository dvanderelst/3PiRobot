...
We trained the inverse model on data from all six sessions. From each session, we held out 15\% of the data in contiguous sets—the data collected nearest a randomly chosen location. We used early stopping on these regions and report performance on both training and held-out data (Table~\ref{tab:inverse-results}). Because the held-out regions are coherent arena patches, they reflect locations unseen during training; a small performance gap indicates minimal overfitting. We deployed the model trained on the remaining 85\%. A single held-out region of this size provides a noisy estimate, particularly for the pole class ($175$ of the $418$ held-out echoes). As a robustness check, we also cross-validated the architecture and procedure over the four spatial quadrants, scoring each echo once while held out. We report those figures alongside the deployed model where informative. We did not use a separate validation set; instead, we consider successful task completion as model validation.
...
As a comparison, besides the sonar-only runs, we ran runs using vision via the overhead camera system. At each step, regardless of modality, the robot received a single local feature description of the nearest reflector within its forward $\pm 35^{\circ}$ field. This description included the reflector's class and matching geometry: pole azimuth and range for poles, and a three-direction depth profile for walls. For sonar, we used the inverse model's output for the binaural emission acquired at the robot's current pose. For vision, we used the overhead camera's digitized arena geometry at the robot's current pose, taking the nearest reflector in the same $\pm 35^{\circ}$ cone. Crucially, we supplied only this local feature from the robot's instantaneous pose, never a map of the arena. Although the overhead camera held the complete arena geometry, we restricted the controller to querying it only within the same forward cone as the sonar inverse. Thus, the overhead camera system modeled a bat's local visual field.

Our vision model made perception stroboscopic for both modalities: the robot perceived the environment once per step. Although bat sonar is inherently stroboscopic, vision is not, we nevertheless modeled vision as intermittent to match the sampling rate of the two conditions. The modalities were not matched in range, however. We did not range-limit vision, whereas the sonar inverse grows unreliable with distance: it was trained on echoes out to $3.2$\,m, but its discrimination of the nearest reflector's class degrades beyond roughly $1.4$\,m. We therefore refused a pole call whenever the class-agnostic range head placed the nearest reflector beyond $1200$\,mm, so that the robot could not act on an identification made where walls and poles are no longer separable. The wall channels carried no such veto, since the depth profile is used to steer away from a surface rather than to identify one. This range asymmetry reflects the difference in operational range between sonar and vision \citep{Boonman2013}.
