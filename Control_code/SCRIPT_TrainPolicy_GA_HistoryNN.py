...
class HistoryNNPolicy:
    """
    Small MLP policy mapping history features to rotate2.
    Input per history step: [iid_norm, dist_norm, prev_rot_norm, prev_drive_norm, prev_blocked]
    """

    def __init__(self, max_rotate2_deg: float, deadband_db: float, history_len: int, hidden_sizes: Tuple[int, int]):
    ...
    def rotate2_deg(self, hist_vec: np.ndarray, current_iid_db: float) -> float:
        ...
        rot_norm = float(np.clip(y[0, 0], -1.0, 1.0))
        return rot_norm * self.max_rotate2_deg
...
class Evaluator:
    ...
    def episode(self, policy: HistoryNNPolicy, start: Tuple[float, float, float]) -> Dict[str, Any]:
        ...
        for t in range(self.cfg.max_steps):
            ...
            current_feat = np.array([iid_norm, dist_norm, prev_rot_norm, prev_drive_norm, prev_blocked], dtype=np.float32)
            ...
            if pad_n > 0:
                hist_vec = np.concatenate([np.zeros((pad_n * 5,), dtype=np.float32)] + [h for h in hist], axis=0).astype(np.float32)
            else:
                hist_vec = np.concatenate(hist, axis=0).astype(np.float32)
            rotate2 = policy.rotate2_deg(hist_vec, iid)
            action = {
...
def plot_policy_curve(policy: HistoryNNPolicy, cfg: Config, output_dir: str) -> None:
    os.makedirs(output_dir, exist_ok=True)
    ...
    for d in dists:
        ...
        feat = np.array([iid_norm, np.full_like(iid_norm, dist_norm), np.zeros_like(iid_norm), np.ones_like(iid_norm), np.zeros_like(iid_norm)], dtype=np.float32).T
        # Repeat the same feature row across the history window for a probe view.
        rot = np.array([
            policy.rotate2_deg(np.tile(frow, cfg.history_len).astype(np.float32), float(iv))
    ...
    ax.set_title("History-NN probe: IID vs rotate2 at fixed distance")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
...
def write_overview(cfg: Config, output_dir: str) -> None:
    ...
    lines = [
        ...
        "- policy is a small MLP that outputs rotate2 from history features",
        "- per-step features: [iid, distance, prev_rotate2, prev_exec_drive, prev_blocked]",
...
def main() -> None:
    ...
    print("=" * 60)
    print("Policy: rotate1=0, drive=100mm, rotate2 from history NN.")
    ...
    best_policy = HistoryNNPolicy(cfg.max_rotate2_deg, cfg.iid_deadband_db, cfg.history_len, cfg.hidden_sizes)
