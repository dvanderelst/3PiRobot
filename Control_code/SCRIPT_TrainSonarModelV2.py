#!/usr/bin/env python3
"""
SCRIPT_TrainSonarModelV2.py

Same SonarSlicesUQ training pipeline as SCRIPT_TrainSonarModel.py, but
sourced from the new AcquisitionSessions/<name>/ format (output of
SCRIPT_VisualDataAcquisition.py) via Library.AcquisitionSessionLoader.

Differences from the original trainer (everything else is identical):
  - SESSION_PATHS → ACQUISITION_SESSIONS (list of folder names under
    AcquisitionSessions/).
  - VALIDATION_QUADRANTS keys are session names from the new layout. With a
    single-session validation run, hold out one quadrant (~25% of pings).
    Loader's quadrants are by per-session median split on (x, y), so each
    quadrant is approximately equal-count regardless of arena shape.
  - OUTPUT_DIR / ARTIFACT_PREFIX point at SonarModelV2/ during validation
    so we don't clobber the existing deployed artifacts. Once V2 is
    validated and the original trainer is retired, restore these to the
    canonical paths.

Outputs in SonarModelV2/  (prefix `slices_`):
...
from Library.AcquisitionSessionLoader import load_data as load_acquisition_data
...
def load_data():
    """Same return signature as the legacy trainer's load_data:
        sonar (N, T, 2), profiles (N, profile_steps), quads (N,), sess (N,),
        bin_centers (profile_steps,)
    Driven by AcquisitionSessionLoader so the source format is the new
    AcquisitionSessions/<name>/ layout."""
    return load_acquisition_data(
