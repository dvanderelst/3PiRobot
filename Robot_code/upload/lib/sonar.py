...
def acquire(mode, sample_rate=10000, n_samples=100):
    ...
    # Keep receivers in receive-only mode
    ...
    buf0 = array.array('H', (0 for _ in range(n_samples)))
    buf1 = array.array('H', (0 for _ in range(n_samples)))
    ...
    if period_us <= 0:
        raise ValueError("sample_rate too high for microsecond scheduling")

    next_due = t_gate
    for i in range(n_samples):
        while time.ticks_diff(time.ticks_us(), next_due) < 0:
            pass
        buf0[i] = adc_recv1.read_u16()
        buf1[i] = adc_recv2.read_u16()
