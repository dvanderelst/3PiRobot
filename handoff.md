...
- **Paper:** on branch `direct-learning-poletask`. **Introduction fully reworked (2026-06-20/21).** Now a clean, contiguous **Par 1–8**: overlap → inverse model + asymmetry → cross-modal inverse training (defined in Par 3) → sonar (consolidated) → vision → synthesis → "this paper" (two tasks) → biological-plausibility close (Par 8). US spelling throughout; terminology standardized to *cross-modal inverse training*. See Paper state (2026-06-21) for detail. **Next:** the intro has **no Discussion hand-off** (the direct/vicarious pointer was dropped) — draft the (briefer) Discussion treatment of direct + vicarious learning, drawing on the retired material commented after `\end{document}` (vision-as-internal-model / planning argument, Mugan2020/Bennett2023, `\cnote{5}`); reconcile Par 7 "pole" vs Methods "wooden dowel of 25 mm"; confirm Task 2 wording ("path integration + landmark recognition") matches the actual experiment; trim Par 5 small-mammals/rodents acuity redundancy. **Inverse-model Methods (Par 20–22) + Results (Par 23–27, `tab:inverse-results`, `fig:inverse-results`) are drafted and committed (`b53644e`, `7354057`)** around the B architecture + spatial-holdout deployment model (in-sample vs held-out; no generalization claim). **Experiment 1 Methods (obstacle avoidance + target approach) drafted 2026-06-24, uncommitted** — a new `\subsection` before Results, grounded in `SCRIPT_RunDirectPolicy.py`, with 4 open `\cnote`s. **Experiment 1 trial design is now RESOLVED** (see Code state 2026-07-28) — the blocking `\cnote` can be answered in Methods: 5 tape-marked start poses, 2 pole placements, 9 start×pole trials, each run once under sonar and once under vision, with the pole never visible at step 0 (outside the ±35° cone *or* beyond the 1 m horizon). Report **sim as a third condition**: it establishes the controller's ceiling (1400 rollouts, every interior position reached from every start, zero collisions, zero jams), which is what licenses attributing sonar failures to perception rather than to the reactive rule. Remaining `\cnote`s: summary measures, and constants-in-prose vs a parameter table. Also new: an unfinished `\dnote` at Par 2 on the owl's az/el mapping being a simple 2D→2D map against our object-based inverse — the sentence trails off at "take a look at this paper:". New Experiment-1 paragraphs use descriptive `% comments` (not `% Par N` — renumber pending). See Paper state 2026-06-24.
- **Code:** on branch `direct-learning-poletask`. **Experiment 1 setup is settled and the backlog is committed (2026-07-28, six commits).** `SCRIPT_RunDirectPolicy.py` is finally in history (`e424646`) after five weeks uncommitted — wander + corner-escape + deploy inverse + the tuned constants, committed together because `ReactiveParams` reads them as import-time defaults. Two new scripts: `SCRIPT_DigitizeStartPoses.py` (tracker-recorded start poses) and `SCRIPT_SweepPolePositions.py` (simulates the real controller over candidate pole positions). **The design:** 5 tape-marked starts recorded; walls-only arena built for `TempOutput/StartPositionDigitization`; 2 pole placements **P1 (−1203, −84)** and **P2 (−203, −2084)**, 2236 mm apart, giving **9 trials** (P1 serves all 5 starts, P2 serves S2–S5) = **18 robot runs** with sonar and vision paired per trial, ~701 sim steps per condition. **Trial runner is wired (`a67f68a`):** set `SENSE_SOURCE` / `POLE` / `START` / `ARENA` and everything else derives — `SESSION` becomes `direct_<POLE>_S<START>_<source>` so no two of the 18 runs can collide, and `CONTROLLER_SEED` derives from (POLE, START) but *not* from `SENSE_SOURCE`, so a trial's sonar and vision runs draw identical curl signs and escape re-rolls and are a matched pair. Two guards: an unrecognised (POLE, START) warns against `pole_positions.json`, and a built arena whose pole is >400 mm from the intended placement **refuses to run** (otherwise the referee scores against a pole that is not physically there and nothing looks wrong until analysis). **Next, in order:** (1) place P1 → snapshot, annotate, build, recalibrate → 5 sonar + 5 vision; then P2 → 4 + 4; (2) Experiment 1 Results. Deploy gate unchanged: re-run `SCRIPT_CalibrateRobot.py` before each placement's runs.
