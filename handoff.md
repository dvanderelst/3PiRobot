...
- **Code:** two-headed inverse on Acq01A + Acq02B, close-range (`MAX_RANGE_MM = 1000`). 4-fold CV: 83% class acc ± 5.9, 88% wall recall, 70% pole recall ± 17, pole precision 71% ± 0.8, pole-az RMSE 17.2°. Pole-az regression head doesn't localise but its *sign* (left/right) is ~68% — keep the regression head, threshold at 0 for sign. **Calibration pipeline at the pre-bundle baseline (2026-05-27).** PyLorex `arena-frame-calibration` merged into `main` (fast-forward, 10 commits). Pipeline is: per-camera intrinsics + dot-board PnP + plumb-line `script_set_camera_center.py` for cross-camera alignment + ray-plane marker projection in `Lorex.get_aruco`. Wall-probe verification on 2026-05-27 across 6 robot-flush-against-wall positions: per-camera biases between −30 and +72 mm, mean ≈ +25, |bias| under ~75 mm. Today's plank bundle-adjustment experiment (`script_calibrate_plank.py`) was tried as a primary calibration path and rolled back: it produces an internally consistent calibration but Environment.py's warp pipeline still uses `H_raw` while the runtime tracker uses ray-plane, so adopting the bundle results breaks `arena_features.npz` overlay (100–200 mm wall biases). The plank tooling is committed and is now documented in `PyLorex/Docs/calibration_process.md` as **Phase 5** (recommended quantitative checker) and **Phase 6** (deferred alternative calibration source — blocked on the warp refactor). **Next session:** either resume data acquisition / training on the current calibration, or take on the Environment.py warp refactor to unblock Phase 6.
- **Tomorrow first actions** (before any data acquisition):
  - On the **server computer** (not this one): `git pull` PyLorex `main` to pick up today's commits (calibration docs + design notes), then restart the tracking server so it loads any updated runtime code. Calibration *files* are unchanged from the 2026-05-26 state, but a clean restart removes any doubt about cached state.
  - On the **robot/control computer**: run `PyLorex/PyLorex/script_capture_wall_probe.py`, drive the robot flush against 2–3 walls. Per-camera biases should land in the ~−30 to +75 mm band seen yesterday. If they're substantially worse (>>100 mm), the server is on stale state — investigate before acquiring data.
  - If both checks pass, resume Acquisition03/04 collection.
...
- **Collect more acquisition sessions, then deprecate Acquisition01A.** Canonical set is currently `[Acquisition01A, Acquisition02B]`, but 01A predates the new 81 mm marker and mixes old-marker tracking quality with the rest. Plan: once ≥2 more new-marker sessions are in (Acquisition03/04, currently being collected), drop 01A so the canonical becomes all-new-marker (e.g., `[Acq02B, Acq03A, Acq04A]`). The persistent q=0 fold weakness may partly be a 01A tracking artifact and could resolve when 01A is dropped — if it doesn't, the spatial-quadrant cause is something else.
...
- **Camera-system calibration (2026-05-26, mostly resolved).** Geometry now anchored to physical measurements via three JSON files under `PyLorex/Calibration/Results/`:
  - `c_measured_{tiger,shark}.json` — each camera's nadir + height, **in that camera's own per-camera board frame**. Derived from clicking the floor plumb-line mark in the stitched arena snapshot + tape-measured `Cz`. Loaded as `bundle["C_measured"]` by `CalibIO.load_pose_bundle`; consumed by the calibration script (not by the runtime tracker — see Marker-height correction item).
  - `camera_system.json` — derived `shark2tiger_delta_{x,y}` from the two `c_measured` files + the user-entered physical inter-camera distance (2180 mm). `Settings.py` reads this at import (with hardcoded `(0, -1400)` as fallback). Important: the stale hardcoded `-1400` was wrong by ~520 mm; the correct delta from the 2.18 m anchor is ~−1920.

  All three files are written in one workflow by `script_set_camera_center.py`. Re-run when the cameras (or their plumb marks) move.

  After the calibration overhaul, cross-camera agreement (`script_check_camera_agreement.py`, 15 manual placements in the FOV overlap): mean `dy` ~0 mm with ~70 mm scatter, `dx` linear-with-x gradient (range ~175 mm, slope ~+0.07 mm/mm), `|Δ|` 15–40 mm in the arena interior, up to ~125 mm at the x extremes. Was 300–400 mm before the unification fix and 100–260 mm after the unification fix but before the H_raw → ray-plane switch.

  **What's not (yet) fixed.** The residual `dx`-linear-with-x gradient is most plausibly a small relative rotation/tilt mismatch between the two cameras' PnP-derived `R` values (the sign of the `dx(x)` slope flipped between H_raw and ray-plane, which means neither calibration is right at the periphery — the truth is between them). Full bundle adjustment that ties R, t, K, dist consistently to the plumb-line ground truth would fix it but is a big job; the current ~125 mm worst-case is calibration-grade for the pole/wall paper work.

- **Plank-based bundle calibration — implemented, deferred (2026-05-27).** Originally planned as the canonical replacement for dot-board PnP + `shark2tiger_delta`. Implementation lives in `PyLorex/script_capture_plank.py` (3-marker plank at offsets −500 / 0 / +500 mm, auto-capture, median-of-N), `PyLorex/script_diagnose_plank.py` (residuals of pairwise distances vs ground truth), and `PyLorex/script_calibrate_plank.py` (regularised bundle adjustment refining K + dist + R + t jointly, tiger-anchored; empirically the markers were mounted +90° to the plank — `MARKER_TO_PLANK_YAW_DEG = 90.0`). The bundle converges cleanly (init RMSE ≈ 12 px, final ≈ 1 px with regularised intrinsic refinement; plank-residual std ≈ 25 mm). **Why it was rolled back:** adopting the bundle outputs in production requires `Environment.py`'s warp / `Control_code/SCRIPT_BuildArenaGeometry.py` to also switch from `H_raw + λ` to ray-plane projection. Without that, the warp uses one frame and the runtime tracker uses another, and `arena_features.npz` overlaps the actual walls by 500–700 mm at the periphery (wall-probe biases jumped to 100–200 mm during the experiment). Full procedure documentation now lives in `PyLorex/Docs/calibration_process.md` as Phase 5 (plank as a *checker*) and Phase 6 (plank as the calibration *source* — deferred). To unblock Phase 6, refactor the build/warp path to ray-plane intersection consistently with `Lorex.get_aruco`, then re-run the plank bundle and adopt its `pose_*_arena_frame.{json,npz}` + `camera_system_arena_frame.json` outputs.

- **PyLorex calibration tooling (added 2026-05-22 → 2026-05-26).**
  - `script_collect_intrinsics_live.py` (2026-05-22) — hands-free auto-capture intrinsic calibration with audio cues, live on-screen coverage map, incremental load-existing-images so you can fill in gaps over multiple sessions; auto-saves `intrinsics_{cam}.yml` + `coverage_{cam}.png`.
  - `script_check_camera_agreement.py` (2026-05-22, refined 2026-05-26) — manual-placement two-camera floor-projection agreement diagnostic. Static header now prints PnP `C` and measured `C` side-by-side per camera (with `<-- used by live tracker` tag where appropriate). Live loop applies `shark2tiger_delta_{x,y}` internally so the printed `dx, dy` are real cross-camera residuals (without this, `dy` was swamped by the ~1400–1920 mm frame offset). Outputs CSV + 4-panel plot (`dy(x)`, `dx(y)`, `dx(x)`, spatial `|Δ|` map).
  - `script_run_homography_constrained.py` (2026-05-22, deleted 2026-05-26) — PnP with known camera height as a hard constraint via scipy least-squares; tried against shark, didn't help (error wasn't only in depth), file removed.
  - `script_run_homography.py` (modified 2026-05-26) — added per-dot H_raw fit diagnostics block: inlier count, inlier residual mean/max/RMS in mm, worst-fitting dot location, outlier list. Confirmed both cameras' H_raw fits to sub-mm at the dot grid (tiger RMS 0.23 mm, shark 0.42 mm, 100% inliers each). Naming nit: `estimate_homography`'s `ransac_thresh_px` arg is actually in *destination* units (mm here), not pixels — noted in the comment.
  - `script_set_camera_center.py` (2026-05-26) — the camera-system calibration script (see Camera-system calibration item above for what it writes). Self-contained: captures fresh env snapshot internally, walks the user through clicking each camera's plumb-line floor mark, entering tape-measured `Cz`, then entering the physical inter-camera distance. One workflow, no manual Settings.py edits.
  - `script_generate_aruco_markers.py` (2026-05-26, on branch `arena-frame-calibration`) — generates per-ID ArUco SVGs + tiled master sheets for laser cutting. Three stroke colours (red = sheet cut, blue = tape-registration, green = tape-only cuts) so the marker piece stays clamped until the final red cut. Output under `PyLorex/Markers/generated/`, gitignored. Originally intended for a 25-scattered-marker calibration plan; the plank-based approach only needs 2 of these markers (mounted on the plank ends), so we'll cut just those two.
...
- **Uncommitted working-tree changes (long-standing user WIP, do not bundle into agent commits):** `Library/Settings.py` (rotation table + drive curl/scale, ongoing), `Library/DataProcessor.py`, `SCRIPT_CalibrateRobot.py`, `SCRIPT_RunPolicy.py`, `SCRIPT_TakeEnvSnapshot.py`, `SCRIPT_TrainPolicy.py`, `SCRIPT_BuildAcquisitionPlan.py`, `SCRIPT_BuildArenaGeometry.py`, `SCRIPT_ProbeTrackerAtWall.py`. Also `Paper/main.tex` carries the user's Par 10 Holland/Finger insertion uncommitted. Always inspect `git diff` before staging anything in this repo.
