...
### Two-column layout, and the Introduction rebuilt on the proposal's evidence (2026-09-21; commits `7ebf878` -> `a165af8`)

**Housekeeping.** `texput.log` and `Robot_pictures/` (635 MB of raw photos) are ignored. Figure SVG sources, `_numbers.json` sidecars, the final 2026-08-25 calibration in `Settings.py`, and the Acquisition06 plan settings are committed (`342e108`). Session constants in the `SCRIPT_*` files remain deliberately uncommitted, as before.

**Two-column (`50c97fd`).** Staying on `article`; REVTeX and elsarticle were considered and declined until a journal is chosen. Every float is `figure*`/`table*`, because all were drawn at full text width. **The float cascade has a two-column twin:** page-wide floats answer to `\dbltopfraction`, not `\topfraction`. At its default 0.7, `fig:arena_layouts` was deferred and dragged figures 2-11 to the end of the document. It is now 0.85 beside the existing line. Three floats overran the page and were resized: `fig:direct-results` to 0.7\textwidth, `fig:inverse-results` to 0.86, and `tab:controller-params`' text column to 9.5 cm. Table captions now sit below the tabulars. `h` and `b` are ignored on starred floats; `stfloats` would allow bottom placement if wanted.

**The NSF proposal is a source for this paper.** `/home/dieter/Dropbox/CrossModalProject/main.tex` holds carefully vetted material on what vision delivers to bats and on why sonar is hard. Reuse it condensed; do not re-derive it. Lines 98-251 were read this session. Aims 1 and 3 and Broader impacts were not, and are not relevant here.

**Introduction changes, in reading order.**
- **Taylor2006 (`67a11a8`).** The citation now follows "therefore likely to be more demanding" as neural corroboration (perirhinal cortex against pSTS). It used to sit between the premise and that conclusion, where it read as evidence for a difficulty claim the paper never makes.
- **A bat paragraph of its own (`1d55be6`, `902c563`).** Hoffmann2016: a map of sound *azimuth* in *P. discolor* SC, in register with the visual map. Say azimuth, not space; elevation is not mapped on the same axis. No such map is known in insectivores (Allen2021, the Moss-lab review, cited only for that species contrast; it contains nothing on bats learning an az/el map). **Hoffmann's hypothesis that insectivore vision is too weak to instruct a map is a potential objection to the whole paper, and the text answers it two ways:** absence of evidence, and Dieter's point that a direction map only suits isolated reflectors, whereas most echoes are blends from extended objects. Hoffmann's map was itself measured with single virtual sources. This now motivates the map-based versus object-based contrast, which used to be asserted without a reason.
- **Sonar paragraph (`eca3580`).** "Rich but hard", condensed from the proposal: routes in darkness, kilometre-scale navigation on sonar alone, so the information is in the echoes; but single-target accuracy does not give layout, natural echoes overlap, integration is 200-400 us and angular resolution 40 deg. A sentence that was duplicated verbatim two paragraphs later was dropped.
- **Vision paragraphs (`8dc9914`; bib entries `73a931e`).** They replace the old acuity paragraph and its cnote. The main claim, agreed with Dieter: bats see well enough to perceive the *layout* under natural light. It is tied to the robot: at 0.6 cpd a 25 mm pole is resolved to ~1.7 m, about the range over which the inverse recognises poles. **Vision need not supply range, because echo delay does; what it labels is which kind of reflector lies where.** Do not lean on stereopsis; it has not been demonstrated in any bat. Concurrency at dusk and vision's precedence in conflicts are stated. Bell1986 check: 3'38", 15' and 60' are correct (*Macrotus*, *Antrozous*, *Eptesicus*), but acuity retained at starlight holds for the two gleaners only; the old text claimed it for all three.
- **Closing paragraphs (`c529893`, `a165af8`).** The two tasks are now named, and the test is stated as sufficiency for behavior, not accuracy. This is where the *imperfect-but-sufficient* result lives now that the title has dropped it; the abstract should echo it. "Proof of principle" is unified with the title (closes the small item left open 2026-08-26), and "train", not "calibrate". The feasibility claim is scoped: the robot's sonar is worse than a bat's, **but its teacher is a noiseless best case**, so the claim is "a limited sonar can be taught, given a reliable teacher". The Discussion should pick this up.

**Flagged for Dieter, unresolved.**
- Rydell2003 is cited for "cannot support pursuit of small insects", but as recalled (not opened this session) the paper shows vision helping with *large* conspicuous moths, so it supports the sentence only by contrast. It may want a qualifier here and in the proposal.
- The closing concession ("speak to but do not settle") may be too modest for an Introduction.
- The final paragraph still opens with "biologically plausible", which sits awkwardly beside the scoped ending.
- "(see below)" in the bat paragraph is a bare forward pointer.

**Commit mechanics worth reusing.** Dieter's uncommitted intro edits shared `main.tex` with every change above. Each agent commit was staged by applying the edit to the `HEAD` blob and running `git update-index --cacheinfo`, leaving his hunks unstaged. This is cleaner than reverting and restoring his hunks.

### The control is written and figured, and the paper is retitled (2026-08-26; commits `16f7c62`, `7a15a87`, `3b859df`)

Discharges the one revision the entry below asked for. **Experiment 2's control subsection is now DONE** and `fig:sensory-clamp` carries both conditions, so **every Results subsection in the paper is finished.**

**The text.** The single control paragraph became four: the shared recipe (both conditions draw from the same 500-prediction pool, at every step); what separates them and why both are reported; the numbers; and a two-sentence conclusion. Dieter's own uncommitted rewrite of that paragraph was in the working tree and its opening sentence was kept — the rewrite superseded rather than replaced it, which is the one case the don't-bundle-the-user's-WIP rule cannot separate. Three things the text deliberately does **not** do, all of them traps the entry below names: no directional claim between the conditions (the means flip between arenas and both sets are quoted); *shuffled input*, never *clamp*; and **no appeal to the splicing argument**, so `shuffle_agn_only` and its weak Path07 simulation never enter the paper. Reporting both conditions answers that objection empirically instead — which is the reason to keep both, and the reason not to "simplify" later by dropping one.

**The figure is 1x4, not the 2x2 it was first built as.** Panels pair the two conditions *within* each arena (A/B Arena 1, C/D Arena 2), because the comparison that carries the argument is condition-within-arena. Colours are matched between an arena's two panels: run *i* of the two conditions shares a `CLAMP_SHUFFLE_SEED`, so those two runs saw the same substituted measurements and differ in one channel. The 2x2 survives as `--layout=grid` and the old 1x2-with-demoted-shuffle as `--layout=rows`.

**Two figure traps worth knowing.** (1) **The whitespace was not the layout.** The 2x2 looked half-empty because matplotlib's default axes margins leave a wide band of background around each arena; the new `set_limits()` clips every panel to the wall bounding box and applies to all three layouts. Any future arena figure built from this script inherits it. (2) **It was landing on a float page, alone.** Declared at the very end of the section there was no text left to accompany it, and at half a page it qualified for a float page. Fixed by moving the declaration two paragraphs earlier *and* dropping `p` from `[!htbp]`. Figures 1-10 were checked before and after and none moved, so the `\topfraction` cascade did not fire — **check that mapping again after any float change**, it is two lines of `pdftotext`.

**The paper is retitled.** *Vicarious sonar learning in a bat-inspired robot* -> **"Vision as a scaffold for echo interpretation: a proof of principle in a bat-like robot"** (set with `\\` at the colon so it breaks over two lines). The old title promised the one thing the Results do not deliver: vicarious learning survives only as conditions 6-7 of the Discussion's framework, the tier explicitly beyond what is demonstrated, so the title was the last place it led the paper. Dieter picked this from a shortlist; the runner-up framing, which the chosen title drops, was the *imperfect-but-sufficient* one ("a noisy inverse is enough"). **That half of the result now has nowhere to live except the abstract** — if it matters, the abstract has to carry it, and has to name the two tasks the inverse was sufficient *for* (approaching an object; holding a route on landmark geometry), or "sufficient" reads as a much bigger claim than two tasks support.

**Left open, small:** the title says *proof of principle* while Intro Par 8 says *proof of concept*. Unify on one when the intro is next touched.

### Experiment 2's control becomes two conditions, not one (2026-08-25, later)

The partial sensory clamp flew on both arenas (Performance notes 2026-08-25 (later)), and Dieter's call is to **report both it and the full shuffle**. That changes the control subsection and `fig:sensory_clamp`, neither of which has been rewritten yet. **-> Both were done 2026-08-26; see the entry above. What follows is still the reasoning behind the decision, and the wording cautions still bind — but read the `fig_sensory_clamp.py` paragraph as history, the script has since been rebuilt.**
