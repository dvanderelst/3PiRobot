...
- **Code:** on branch `direct-learning-poletask`. **Experiment 1 is DONE — all 20 robot runs complete (5 starts x 2 pole placements x sonar/vision), written up in the paper in a separate session.** **Experiment 2 (path following) is now UNBLOCKED**: all three blockers recorded on the morning of 2026-08-07 were cleared the same day (see Code state 2026-08-07). The simulator runs on a fitted inverse *error model*, the policy observation carries the class and pole channels, `default_Path02` is trained, and the deploy chain loads and steps end to end.
  **Next, in order — REVISED 2026-08-08.** The horizon result (Performance notes 2026-08-08) changes the order below: the inverse now comes *before* the path redraw, because how far the robot can see determines how much clearance a usable path can afford. Do not redraw against a 1 m horizon.
  ...
  0. **Land the extended-horizon inverse.** (a) Mask the pole-range head to poles within 1 m — range *and* class, currently class only — leaving the classifier full-range; without this the terminal stop's +149 mm near-range bias drives the robot to ~350 mm true against a 500 mm nominal stop, into the sub-253 mm regime with no training data. (b) Verify on 4-fold CV that the bias returns to ≈+86 mm or better and the classification gains hold. (c) Train the deployed model with `main_deploy()` and `FAR_LABEL_MODE="true_class"`. **Nothing deployable exists yet.**
  0b. **Then re-run Experiment 1** on the new inverse (user's call 2026-08-08: the cost is low and the result should improve — first perception moves out from 798 mm and the 800–1000 mm blind ring goes away). This also avoids the paper having to describe two differently-scoped inverses. Note the protocol constants: the runs used `APPROACH_STOP_MM=400` / `ALIGN_MIN_DETECTIONS=3`, while HEAD carries 500 and 6.
  0c. **Optional but principled:** one acquisition session in open space would give `none` real training data beyond 1.7 m and restore the abstain guard, instead of living with 4.4%-of-Path01 exposure. The guard cannot be recovered by moving the cut — at 1700 mm the abstain class has 11 examples and is ignored entirely.
  1. **Then redraw the path.** `default_Path02_run02` completed **2.1 laps (110 steps)** with lap 2 tracking lap 1 to 116 mm, then **crashed at steps 116/117** near (-227, -1120). The cause is geometric, not control:
     ...
     **This is the direct cost of why Path02 was chosen.** It reached more informative perception than Path01 precisely by hugging walls and blocks; Path01's min clearance is 249 mm (164 mm usable) against Path02's 155 mm (70 mm). Closer to obstacles = better sonar = less room for error. **Redraw with a minimum clearance of ~350 mm** (about Path02's current median), i.e. the 266 mm p90 tracking error plus the robot radius, then re-measure informativeness. Use `SCRIPT_DefinePath.py`; it already draws the 800 mm pole landmark ring. **2026-08-08: this trade-off is much weaker at the extended horizon** — Path01 (249 mm clearance) reaches 87.1% informative at 1400 mm and 95.6% at 1700 mm, against Path02's 85.5% at 1000 mm, so a 350 mm-clearance path no longer implies giving up perception. Draw it, then measure it at the deployed horizon before training against it.
