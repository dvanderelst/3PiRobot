...
- **Code:** on branch `direct-learning-poletask`. **Both experiments' robot work is COMPLETE.** Experiment 1: 20 runs 2026-07-30, re-run on the current inverse 2026-08-21 (sonar arm only, 10/10). Experiment 2: two arenas, a replicate on each, pole removal, pole displacement ×2, wall displacement. The 2026-08-08 plan that carried this section for two weeks is fully discharged — extended-horizon inverse (`67c68c0`), Acquisition06A, the retrain on Acq01A–06A, the class-agnostic range head, Path04 v3, the per-episode rotation bias (`db22a14`), the trained policies, the robot ablations, and the path redraw. The reasoning and every number live in Code state and Performance notes 2026-08-08 → 2026-08-21; do not re-derive them here.
  ...
  **What is actually open, in rough priority.**
  ...
  0b. 🔴 **Run the sensory-clamp control condition (2026-08-24, Dieter's call).** `CLAMP_SENSING = "const"` in `SCRIPT_RunPolicy.py` (added `e8e481d`) replaces every feature the inverse reports with a fixed mid-range reading, so the recurrent controller runs on its own dynamics and the rotation it last commanded. **One run, Arena 1 / Path04**, chosen because `SCRIPT_Ablations.py`'s `dead_reckoning` already ran exactly this in simulation on that policy: **0.36 laps, 24/24 collisions, 312 mm cross-track** against 0.99 / 3-of-24 / 89 mm intact. So the robot run confirms a standing prediction rather than standing alone. **Expect collisions — run it last in the session**, after the furniture positions stop mattering. A loud banner prints at startup so a clamped run cannot be mistaken for a normal one, and the setting is captured in the run's own code zip.
     ...
     **Why it is worth table time.** It retires a paragraph. Dieter commented out the "the controller's input has no null value, so sensory data cannot be withheld, only replaced with a false reading" justification in Experiment 2's Methods as too convoluted; one run makes the point empirically instead. The second-arena sentence went into that same comment block as collateral and was restored separately (`3803a37`).
