...
- **Code:** two-headed inverse now trained on Acq01A + Acq02B (02B is a 02A redo with the new 81 mm marker; 02A was cut off mid-run), restricted to close-range pings (`MAX_RANGE_MM = 1000` in `SCRIPT_TrainInverseModel.py`). 4-fold CV: 83% class acc ± 5.9, 88% wall recall, 70% pole recall ± 17, pole precision 71% ± 0.8 (cross-fold variance collapsed), pole-az RMSE 17.2°. Pole-az regression head doesn't localise yet, but its *sign* (left/right) is robustly above chance (~68%) — keep the regression head and threshold at 0 for sign; explicit sign-BCE training is worse. **Marker-height bias fixed (2026-05-22, PyLorex `be274f7`)** — `floor_xy_mm` now back-projects through `z = Settings.marker_height_mm = 150 mm` via ray-plane intersection at the camera centre. **Camera extrinsics fully redone 2026-05-22 evening** (cameras physically moved + new intrinsics with deliberate peripheral coverage + single shared dot board for homography). Tiger now PnP-verified accurate (height 2946 mm vs tape 2945). **Shark still has a residual ~100 mm height error** (PnP says 3047–3076 mm) that produces a U-shaped `dy(x)` cross-camera disagreement, peaking at ~150–190 mm at arena extremes. Constrained-PnP attempt (`script_run_homography_constrained.py`, height-locked to 2945) couldn't fix it — RMSE jumped to 6.3 px under the constraint, confirming the issue is either residual shark intrinsic error or a wrong height measurement (dome-camera housing offset unaccounted for). **Planner wall geometry annotation bias** still present (annotations drawn 130–260 mm outside foam-facing surface) — independent issue, pending re-snapshot + re-annotate after the calibration is settled. Next: physically remeasure shark's height (account for dome housing thickness, ~30–50 mm typical), and/or redo shark intrinsics with deliberate sampling specifically at the dot-board region; then retry the homography and re-run `script_check_camera_agreement.py` to verify the U-shape has collapsed.
...
- **Marker-height bias — fixed 2026-05-22 (PyLorex commit `be274f7`).** Original bug: tracker reported `floor_xy_mm` via `pixel_to_board_xy(u, v, use_raw=True)` which back-projects through `H_raw` (a homography fitted on a board *on the floor*). For a marker physically at z = 150 mm (caliper-measured 2026-05-22), this introduced an XY bias pointing away from the camera nadir, magnitude `mz/(cz − mz) × distance_from_nadir ≈ 40–80 mm` at typical distances. Fix applied: when extrinsics are known, `floor_xy_mm` is computed by ray-plane intersection at `z = Settings.marker_height_mm` using the camera centre `C = -R.T @ t` — the same `λ = 1 - h/Cz` formula the wall back-projection uses. **Verified empirically:** parked the robot against the north wall at the same physical position before/after the fix; reported `y_mm` shifted 1224 → 1172 (≈50 mm toward camera nadir), matching the predicted bias correction. Settings exports `marker_height_mm = 150.0`, `marker_height_tolerance_mm = 20.0`; `height_mm` field is reported as `Settings.marker_height_mm` (the assumed height used for back-projection).
...
- **Planner wall geometry is systematically wrong.** Probed N wall (multiple x) and W wall (`SCRIPT_ProbeTrackerAtWall` 2026-05-22): physical walls sit 130–260 mm *inside* (toward arena centre) where the planner has them. Magnitude varies by position. Direction is consistent: annotation polylines were drawn at a wall-structure feature beyond the actual foam-facing collision surface (likely the panel back / framework outer edge, visible from above behind/above the foam). Caused at least one Acquisition02 west-wall collision and ongoing planner-vs-physical mismatch at every wall touch point. **Acquisition03 acoustic data is unaffected** — the sonar recorded what the robot actually saw; only the planner's idea of "where the walls are" is biased.
- **Camera FOV at z = 0 didn't reach far enough north** to capture the foam wall structure in the rectified arena image. Expanded `environment_arena_max_y_mm` 1500 → 2000 and `min_y` -3000 → -3500 (PyLorex `616a5e7`); the bounds change is in effect but the camera FOV itself is the limit, not the bounds. **Cameras physically repositioned 2026-05-22.**
- **Calibration state (post-2026-05-22 evening session).** Intrinsics redone for both cameras with **deliberate peripheral coverage** using new `script_collect_intrinsics_live.py` (live hands-free capture with audio cues + on-screen coverage map; tiger RMS 0.23 px @ 60 samples, shark RMS 0.20 px @ 43 samples — both with corner/edge coverage). Homography redone with a **single shared dot board** (initial naïve "two boards, translation-only between them" approach gave a 1300 mm world-frame offset — they have to be the same board in the same physical spot). **Tiger now PnP-verified accurate:** height 2946 mm vs tape-measured 2945 mm, RMSE 1.7 px. **Shark still has ~100 mm residual height bias** (PnP 3047–3076 mm), driving a quadratic `dy(x)` cross-camera disagreement that peaks at ~150–190 mm at arena extremes. New diagnostic `script_check_camera_agreement.py` measures this. Constrained-PnP attempt (`script_run_homography_constrained.py`, height locked to 2945) couldn't fix it: RMSE jumped 2 → 6.3 px under the constraint and the U-shape barely moved — confirming the underlying issue is either (a) residual shark intrinsic error in the dot-board image region, or (b) the 2945 measurement is wrong for shark (dome-camera housing offset, optical centre is typically 30–50 mm inside the dome).
- **Next-session pipeline (post-calibration).** (1) **Tomorrow first**: physically re-measure shark's optical-centre height (be precise about the reference point on the camera — not the dome bottom, estimate where the lens / sensor actually sits). If shark turns out to be at ~3000–3050 mm physically, the unconstrained PnP was approximately right and we accept the current calibration as the limit of what's achievable without more intrinsic work. If shark really is at 2945, the residual U-shape is intrinsic-driven and shark needs another intrinsic pass focused on the dot-board image region. (2) Re-run `script_check_camera_agreement.py` after any change; the diagnostic CSV in `Temp/camera_agreement.csv` is the verification step. (3) Re-take environment snapshot via `SCRIPT_TakeEnvSnapshot.py` and confirm the foam wall structure is fully visible in the rectified `arena_tiger.png`/`arena_shark.png`. (4) Re-annotate with green polylines **traced along the foam-top edge**, not along the panel back / framework outer edge. (5) Re-run `SCRIPT_BuildArenaGeometry.py`. (6) Use `SCRIPT_ProbeTrackerAtWall.py` to verify the new planner-walls match physical reality. (7) Then resume Acquisition03/04 collection.
- **New PyLorex calibration tooling (added 2026-05-22 evening).** `script_collect_intrinsics_live.py` — hands-free auto-capture intrinsic calibration with audio cues, live on-screen coverage map, incremental load-existing-images so you can fill in gaps over multiple sessions; auto-saves `intrinsics_{cam}.yml` + `coverage_{cam}.png`. `script_check_camera_agreement.py` — manual-placement two-camera floor-projection agreement diagnostic, ~15 robot placements with audio cues, outputs CSV + 4-panel plot of `dy(x)`, `dx(y)`, spatial map. `script_run_homography_constrained.py` — PnP with known camera height as a hard constraint via scipy least-squares (Rodrigues + 2D translation, height fixed). Useful when the planar-PnP focal-length / depth ambiguity is suspected to bias depth.
