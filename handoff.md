...
- **Paper:** on branch `direct-learning-poletask`. **Introduction fully reworked (2026-06-20/21).** Now a clean, contiguous **Par 1–8**: overlap → inverse model + asymmetry → cross-modal inverse training (defined in Par 3) → sonar (consolidated) → vision → synthesis → "this paper" (two tasks) → biological-plausibility close (Par 8). US spelling throughout; terminology standardized to *cross-modal inverse training*. See Paper state (2026-06-21) for detail. **Next:** the intro has **no Discussion hand-off** (the direct/vicarious pointer was dropped) — draft the (briefer) Discussion treatment of direct + vicarious learning, drawing on the retired material commented after `\end{document}` (vision-as-internal-model / planning argument, Mugan2020/Bennett2023, `\cnote{5}`); reconcile Par 7 "pole" vs Methods "wooden dowel of 25 mm"; confirm Task 2 wording ("path integration + landmark recognition") matches the actual experiment; trim Par 5 small-mammals/rodents acuity redundancy. **Inverse-model Methods (Par 20–22) + Results (Par 23–27, `tab:inverse-results`, `fig:inverse-results`) are drafted and committed (`b53644e`, `7354057`)** around the B architecture + spatial-holdout deployment model (in-sample vs held-out; no generalization claim). **Experiment 1 Methods (obstacle avoidance + target approach) drafted 2026-06-24, uncommitted** — a new `\subsection` before Results, grounded in `SCRIPT_RunDirectPolicy.py`, with 4 open `\cnote`s. **Most important paper next:** decide Experiment 1's **trial design** (the control script runs one trajectory per invocation — settle how many start positions/repeats and which conditions: sonar alone vs sonar+vision±sim), then the summary measure (success rate / steps-to-pole / per-step class agreement / example trajectories), then write Experiment 1 Results. New Experiment-1 paragraphs use descriptive `% comments` (not `% Par N` — renumber pending). See Paper state 2026-06-24.
- **Code:** on branch `direct-learning-poletask`. The cross-modal direct-learning demo (`SCRIPT_RunDirectPolicy.py`) runs on the real robot in `sonar` / `vision` / `sim` modes — a hand-coded reactive rule (approach pole / bounce off walls / scan when empty) on the modality-agnostic local feature, overhead tracker as referee; writes a colored trajectory plot, `trajectory.tsv`, an env/code snapshot, and (sonar) one raw-ping dill per step. **Inverse model finalized (2026-06-23).** Architecture is **B** (`SonarSlicesUQ_Wall3` — single symmetric 3-output wall head, adopted 2026-06-22 over the base side-head + z_sym arrangement; performance-equivalent, simpler to argue). Trained via `main_deploy()` on a **single spatial 15% holdout** (seed 0): we make **no generalization claim** — the held-out region is an overfitting guard only, the experiments are the real test (see [[project_validation_purpose]] in `~/.claude` memory and Performance notes 2026-06-23). Deployed model is `inverse_deploy_*`, loaded with `INVERSE_FOLD = "deploy"`. 4-fold quadrant CV retained in `main()` for diagnostics/EXPT but no longer the deployed/reported path. **Next, in order:** (1) make the controller's `empty`/`none` branch **wander** (curl + drive) instead of scanning in place, so the 1 m sonar horizon doesn't leave the robot spinning in open stretches; (2) **commit the `SCRIPT_RunDirectPolicy.py` working-tree changes** (the `INVERSE_FOLD="deploy"` repoint + earlier deploy-side work) — still uncommitted, entangled with the user's live tuning constants / `SESSION` / `SENSE_SOURCE`; (3) optional: class-weighting or a lowered pole threshold to recover some pole recall. Deploy gate still applies: re-run `SCRIPT_CalibrateRobot.py` before any `sonar`/`vision` run. Old 2-class model backed up at `Control_code/SonarModel.bak_2class/`.
