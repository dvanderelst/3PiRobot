...
- **West-wall discrepancy — root cause identified (2026-05-21), fix pending.** During Acquisition02 the robot approached the west wall much closer than the plan plot suggested and hit it at one waypoint (~y = −1300 mm). Code inspection of `PyLorex/LorexLib/Lorex.py:487` confirms the cause: the tracker reports the marker's `floor_xy_mm` via `pixel_to_board_xy(u, v, use_raw=True)`, which back-projects the marker pixel through `H_raw` — the homography fitted on a calibration board *on the floor*. The marker is physically ~100 mm above the floor, so this introduces a position bias of `mz / (cz − mz) × distance_from_camera_nadir` (~2.7% × distance for typical mounting), pointing **away from the camera's nadir**. On the west side that pushes the robot closer to the west wall than the planner intended; magnitude predicted at ~40–80 mm at ~1500 mm from nadir. Wall polylines are unaffected — they're already back-projected through the wall's top-edge height. Two fix paths (in cleanliness order):
  1. **Use the PnP result directly.** PnP is already computed in the same loop (`t_board_tag` on `Lorex.py:498`) and gives the marker's 3D position naturally. Replace `floor_xy_mm` with `(t_board_tag[0], t_board_tag[1])`.
  2. **Corrected back-projection through `z = marker_height_mm`.** Add a setting, plumb it through `pixel_to_board_xy`, do ray–plane intersection at that height.

  Tomorrow's next step: measure the marker plane height with calipers, then either implement fix (1) directly or do an empirical magnitude check first (compute predicted offset, see if it matches the observed wall-collision overshoot of ~40–80 mm).
