...
- **Code:** two-headed inverse now trained on Acq01A + Acq02B (02B is a 02A redo with the new 81 mm marker; 02A was cut off mid-run), restricted to close-range pings (`MAX_RANGE_MM = 1000` in `SCRIPT_TrainInverseModel.py`). 4-fold CV: 83% class acc ± 5.9, 88% wall recall, 70% pole recall ± 17, pole precision 71% ± 0.8 (cross-fold variance collapsed), pole-az RMSE 17.2°. Pole-az regression head doesn't localise yet, but its *sign* (left/right) is robustly above chance (~68%) — keep the regression head and threshold at 0 for sign; explicit sign-BCE training is worse. **Marker-height bias fixed (2026-05-22, PyLorex `be274f7`)** — `floor_xy_mm` now back-projects through `z = Settings.marker_height_mm = 150 mm` via ray-plane intersection at the camera centre. Verified at north wall (`y_mm` shifts 1224 → 1172 against same physical wall position). **Planner wall geometry is bad** — annotations were drawn outside the actual foam-facing surface; planner walls sit 130–260 mm beyond physical walls. Pending: camera repositioning + re-annotation. Acquisition03/04 acoustic data is unaffected (the bias was on the planner side, not on what the sonar recorded). Next: physically move the cameras so the north wall structure falls inside the floor-plane FOV, recalibrate extrinsics, re-snapshot, re-annotate, re-build geometry — then resume acquisitions.
...
- **Camera FOV at z = 0 didn't reach far enough north** to capture the foam wall structure in the rectified arena image. Expanded `environment_arena_max_y_mm` 1500 → 2000 and `min_y` -3000 → -3500 (PyLorex `616a5e7`); the bounds change is in effect but the camera FOV itself is the limit, not the bounds. **Cameras physically repositioned 2026-05-22** (taken down, re-angled, mounted further apart north/south). Extrinsics calibration is now invalidated and must be redone before any new env capture or arena geometry build.
- **Next-session pipeline (post-camera-move).** (1) Re-run extrinsics calibration via `script_run_homography.py` for tiger and shark, with the calibration dot board freshly placed on the floor for each. (2) Re-take environment snapshot via `SCRIPT_TakeEnvSnapshot.py` and confirm the foam wall structure is fully visible in the rectified `arena_tiger.png`/`arena_shark.png`. (3) Re-annotate with green polylines **traced along the foam-top edge**, not along the panel back / framework outer edge. (4) Re-run `SCRIPT_BuildArenaGeometry.py`. (5) Use `SCRIPT_ProbeTrackerAtWall.py` to verify the new planner-walls match physical reality (robot circle should sit flush against the green wall dots in `TempOutput/probe.png`). (6) Then resume Acquisition03/04 collection.
