...
- **Second arena — Path07. DEPLOYED AND WORKING 2026-08-19: 500 clean steps, 6.44 laps, cross-track median 42 mm against Path04's 36.** Artifact `PolicyTraining/default_Path07/best_policy_survival.json`; deploy config committed at `70edfa1`. **Three manipulations run 2026-08-20, all 500 steps, none with a collision** — poles displaced (a near-null, 42 mm median, ~+28 mm of route shift), P0+P1 removed (route shifts +66 east / +102 north over the north lobe, six laps, never negative, and back at baseline everywhere else), and a wall added *across* the trained path in the south (centreline margin −42 mm after the robot radius; the robot re-routes, closest approach 510 mm, 416 mm further north, no collision). Full numbers, the measured arena inventory and the method that recovers it from `arena.png` are in Performance notes 2026-08-20. **DONE 2026-08-21: the replicate landed at 42 mm against the baseline's 42**, window medians agreeing to ≤15 mm, and the moved-poles condition was re-run with the P2 oversight corrected — bigger displacements (P0 ~800 mm ESE, P1 ~400 mm SW), same near-null (+43 vs +34 mm of route shift). **Experiment 2's robot work is therefore complete**: two arenas, a replicate on each, pole removal, pole displacement ×2, wall displacement. The ladder with the floor under it — displacement of thin dowels ~30 mm (≈2× floor), removal ~110 mm, wall displacement 416 mm — plus the sensory-signature analysis that corroborates the channel ablation on the robot, are in Performance notes 2026-08-21. **What Exp 2 still needs is the write-up, which does not exist: no subsection, no figures.** **EXPERIMENT 1 IS RE-RUN AND DONE (2026-08-21 evening): 10/10 `reached_aligned` on the current inverse**, no collision, jam or timeout, with the cost of sonar roughly halved (median 74 → 39 steps) and pole precision at 94.3%. **Three published numbers change** — the cost-of-sonar figures, "zero bearing corrections in any run" (now three of ten), and the arrival range (188–408 mm, one inside the 253 mm no-training-data band). Full numbers in Performance notes 2026-08-21 (evening). **Both experiments' robot work is now complete; what remains is writing.** Two loose ends before the paper numbers are regenerated: `exp1_stats.py`'s `run_dir()` still resolves `PolicyRuns/direct_…` while both arms now live under `PolicyRuns/Poles/`, and `DATA_FOLDER`/`SUFFIX` in `SCRIPT_RunDirectPolicy.py` should be made to agree with that layout. Historical, kept because it is the reason the re-run worked: the offline replay said **do not re-run on the bare argmax**: the current inverse takes pole precision from 97.4% to **61.4%** on the identical 761 stored envelopes, with 55 false pole calls beyond 1.4 m, and the controller turns toward perceived poles. **DONE 2026-08-21 (`e81afeb`, `fcaf26d`): the veto is landed at 1200 mm and the published terminal protocol restored** — the 400 mm stop, ten corrections and the three-consecutive-detection confirmation rule turned out never to have been committed and had to be recovered from a run's frozen `code_*.zip`; see Code state. **The re-run is ready to go**: set `SENSE_SOURCE="sonar"`, `POLE`/`START`, and `ARENA` to **`DirectPole01`** (P1) or **`DirectPole02`** (P2) — HEAD's `"DirectP1"` does not exist — and work through the ten pole × start combinations, running P2/S5 with `START=5`. Original reasoning for the cap: refuse a pole call when the agnostic range head reads beyond it. **Cap: 1200 mm** (88.2% precision, 800–1000 mm recall 25% → 81%, median first detection 798 → 995 mm, 2 far phantoms against 55 ungated, and every false call at ≥800 mm true range so it self-corrects with runway to spare). 1000 mm is the conservative alternative (95.3%, but gives back half the gain). A confidence threshold alone is a trap — it restores precision by throwing away the range gain. **Vision does NOT need re-running**: `GEOM_RANGE_HORIZON_MM = None`, so vision reads unbounded geometry through `feature_from_geometry`; the two arms were never horizon-matched and the paper says so. Then re-run the **ten sonar runs only**, holding `APPROACH_STOP_MM=400` / `ALIGN_MIN_DETECTIONS=3` (HEAD carries 500 / 6), vision untouched (`CONTROLLER_SEED` excludes `SENSE_SOURCE`, so pairing survives), and run P2/S5 with `START=5` to repair the one mismatched pair. Numbers and the full sweep in Performance notes 2026-08-21 (later); tooling is `EXPT_replay_exp1_inverse.py`. The gate touches only the direct policy — `SCRIPT_RunPolicy` feeds `encode_obs` the raw dict, so Exp 2 is unaffected. **Not planned: a blind-policy run.** Path integration is the substrate the sensing corrects, not a claim the paper makes (Dieter, 2026-08-20); the supported statement is that the route holds across a 3.5 °/step swing in motor bias while the policy integrates only *commanded* rotation. **Walls now have a displacement result too** (the added-wall run *is* a ~420–500 mm inward move of the south boundary, with near-complete route capture), so the remaining wall gap is only a *graded* one: no wall has been moved by an amount small enough to leave the path flyable, and none has been removed. And the class channel keeps testing inert, so what is demonstrated is boundary and object *geometry*, not object identity — which is also the resolution the intro's Task 2 wording needs. **Then re-derive `motion_rot_bias_deg`**: the simulator's cross-track runs 1.4–1.7× the robot's on *both* paths, because ±5 °/step was set from badly-calibrated sessions, so every sim survival figure quoted for Path06/Path07 is probably too low. Full detail in Performance notes 2026-08-19. Path06 is retired to `TargetArenas/old_stuff/`. Three rules that came out of the day and apply to everything downstream: (1) **never judge a policy by `val_mse`**; (2) **check `Settings.py` holds non-identity calibration before every deploy** (`SCRIPT_RunPolicy` now refuses otherwise); (3) **the inverse is blind below ~400 mm**, so the robot has no sonar collision avoidance at contact range. Full detail in Performance notes 2026-08-18 (late). The Path04 landmark series is **not** invalidated by any of this — but do not retrain a Path04 policy without redoing the whole series.
...
- **Code:** on branch `direct-learning-poletask`. **Experiment 1 is DONE — all 20 robot runs complete (5 starts x 2 pole placements x sonar/vision), written up in the paper in a separate session.** **Experiment 2 (path following) is now UNBLOCKED**: all three blockers recorded on the morning of 2026-08-07 were cleared the same day (see Code state 2026-08-07). The simulator runs on a fitted inverse *error model*, the policy observation carries the class and pole channels, `default_Path02` is trained, and the deploy chain loads and steps end to end.
  **Next, in order — REVISED 2026-08-08.** The horizon result (Performance notes 2026-08-08) changes the order below: the inverse now comes *before* the path redraw, because how far the robot can see determines how much clearance a usable path can afford. Do not redraw against a 1 m horizon.
  ...
  4. **Then collect repeats and the ablation conditions**, on the robot rather than in sim (user's call, 2026-08-07): `use_poles=False` and the blind control. The plumbing exists. Whether the *simulated* policy degrades without the pole channel says something about the simulator, not the robot. Retraining is needed for the new path anyway, so fold the training-noise fix (item 6) in at the same time.
     - **DONE 2026-08-14 — the robot-side ablation is landmark removal, not channel clamping, and the four-run design is complete.** baseline 63 / **replicate 64** / pole removed 86 / pole+B1 removed 128 mm median cross-track, 500 steps each. **The replicate is the headline: a run reproduces to +1 mm, so effects down to ~10 mm are measurable and both landmarks are load-bearing** (B1 worth roughly twice the pole). Neither acts locally — each landmark's own window runs at baseline and the cost surfaces downstream. Full numbers, the in-beam/out-of-beam analysis, the three conclusions this reversed, and the class-head negative result in Performance notes 2026-08-14 (late).
     - **DONE 2026-08-17 — landmark DISPLACEMENT, and it is the strongest landmark result so far.** B1 and B2 both moved ~310–330 mm east; the route displaced with them, +107 east / +82 north over 54–75% of the lap against +9/+7 and +13/+14 in the two baselines, **on all nine laps**, with tracking otherwise intact (500 steps, no crash). Roughly a third of the landmark displacement — partial cue capture against the unmoved walls and blocks. This is what removal could not show: the cue is *positional*, not merely load-bearing. Full numbers, the aborted first attempt, and the placement-measurement method in Performance notes 2026-08-17.
     - **Next on this thread, in rough priority.** (a) **A B1-only displacement repeat** — the two boxes moved together, so nothing separates them; the prediction is that the shift roughly halves. (b) **Land `SCRIPT_CheckArenaDelta.py`** (obstacle localisation to ~15 mm — but use the **2026-08-20** method note, not the 2026-08-17 one: plain photo differencing fails across days because the lighting change dominates; per-run colour detection of the dowels is what works) and run it after placing an obstacle, before committing to a 500-step run — this is the cheap fix for the env-snapshot gap, and it would have saved the aborted run. (c) **The flat-panel run**: put an extended reflector where the pole was, at the same range, to change acoustic class while holding range — the only robot test of class-channel inertness, since pole removal does not move p(pole) at all.
