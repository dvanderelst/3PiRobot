...
- **Code:** on branch `direct-learning-poletask`. **Both experiments' robot work is COMPLETE.** Experiment 1: 20 runs 2026-07-30, re-run on the current inverse 2026-08-21 (sonar arm only, 10/10). Experiment 2: two arenas, a replicate on each, pole removal, pole displacement ×2, wall displacement. The 2026-08-08 plan that carried this section for two weeks is fully discharged — extended-horizon inverse (`67c68c0`), Acquisition06A, the retrain on Acq01A–06A, the class-agnostic range head, Path04 v3, the per-episode rotation bias (`db22a14`), the trained policies, the robot ablations, and the path redraw. The reasoning and every number live in Code state and Performance notes 2026-08-08 → 2026-08-21; do not re-derive them here.
  ...
  **What is actually open, in rough priority.**
  ...
  0. 🟡 **Re-running the Experiment 1 sonar arm — IN PROGRESS 2026-08-24.** Ten runs, sonar only; the vision runs stay as they are. Reason unchanged: the 75 mm pole-approach step was missing during the 2026-08-21 re-run, so those ten were not matched to their vision partners (Code state 2026-08-21, late). `SCRIPT_RunDirectPolicy.py` at committed state carries all four restored constants (`POLE_APPROACH_DRIVE_MM = 75.0`, `APPROACH_STOP_MM = 400`, `ALIGN_MAX_STEPS = 10`, `ALIGN_MIN_DETECTIONS = 3`) plus `POLE_RANGE_VETO_MM = 1200`. Order: `POLE="P1"` / `ARENA="DirectPole01"` with `START` 1→5, then `P2` / `DirectPole02` 1→5; only `START` changes within an arena. **The superseded runs were moved before starting**, verified by the `drive_mm` column on `approach` rows: the 150 mm generation is now `old_stuff/direct_*_sonar_repeat01_approach150` (the plain name in `old_stuff` is the 2026-07-30 original), and `PolicyRuns/Poles/` holds only the ten valid 75 mm vision runs. New runs land in `PolicyRuns/` root — move each into `Poles/` as it completes. ⚠️ `DataWriter(autoclear=True)` wipes a session folder silently, so re-flying a trial destroys its predecessor.
  ...
  2. **`exp1_stats.py` cannot read the current layout** — its `run_dir()` resolves `PolicyRuns/direct_…` while the finished runs are filed under `PolicyRuns/Poles/` (a fresh run writes to the root and is moved there afterwards, so both paths exist mid-session). Repoint it, and make `DATA_FOLDER`/`SUFFIX` in `SCRIPT_RunDirectPolicy.py` agree, before regenerating any paper number.
