...
- **Paper:** on branch `direct-learning-poletask`. **Introduction refocused (2026-06-20, commit `90f2a6a`)** around the empirical contribution: a noisy cross-modal inverse model that is nonetheless sufficient to drive behaviour on two robot tasks. The direct/vicarious-learning framing is demoted from a co-equal intro frame to a Discussion topic. New gap paragraph (Par 6) positions prior sonar-classification work (does not drive action). "This paper" paragraph (Par 9) now presents two tasks plainly; retired vicarious-intro material sits in a commented block after `\end{document}`. **Next:** Par 10 typo cleanup, echo2depth `\cnote` forward-ref, confirm Task 2 wording ("path integration + landmark recognition") matches the actual experiment, and write the briefer Discussion pickup of vicarious learning (relocate the retired "vision-as-internal-model / planning" argument, Mugan2020/Bennett2023). Empirical close-range pole/wall result is firming up — see Code state.
- **Code:** on branch `direct-learning-poletask`. The cross-modal direct-learning demo (`SCRIPT_RunDirectPolicy.py`) now runs on the real robot in `sonar` / `vision` / `sim` modes — a hand-coded reactive rule (approach pole / bounce off walls / scan when empty) on the modality-agnostic local feature, with the overhead tracker as ground-truth referee. It writes a colored trajectory plot (markers by *perceived* class), a numeric `trajectory.tsv`, an env/code snapshot, and (sonar) one raw-ping dill per step. **The inverse was retrained with a 3rd `none` class** ("nothing within 1 m") to fix a confident-phantom-pole failure: the old 2-class model, in open space / facing a rear pole, emitted `pole` at p≈0.99. Validated on the saved `direct_pole_demo1` dills — see Performance notes 2026-06-12. **Next, in order:** (1) make the controller's `empty`/`none` branch **wander** (curl + drive) instead of scanning in place, so the 1 m sonar horizon doesn't leave the robot spinning in open stretches; (2) **commit the `SCRIPT_RunDirectPolicy.py` deploy-side changes** — done + validated but uncommitted, must exclude the user's live tuning constants / `SESSION` / `SENSE_SOURCE`; (3) optional: class-weighting or a lowered pole threshold to recover some pole recall. Deploy gate still applies: re-run `SCRIPT_CalibrateRobot.py` before any `sonar`/`vision` run. Old 2-class model backed up at `Control_code/SonarModel.bak_2class/`.
...
7. `SCRIPT_RunDirectPolicy.py` is the Par 9 direct-learning demo: one reactive controller (`ReactiveController`) on a modality-agnostic `LocalFeature` ({pole→azimuth, wall→3 slice depths, empty→scan}), with `SENSE_SOURCE ∈ {sonar, vision, sim}`. Sonar feature comes from `InverseModel.predict_from_envelope` (3-class → `feature_from_inverse`, where class 2 `none` maps to `"empty"`); vision/sim from arena geometry (`feature_from_geometry`, with an optional `max_range_mm` horizon flag `GEOM_RANGE_HORIZON_MM`, default `None` = full sight — training always applies the horizon, deploy vision doesn't need to). Outputs per run: `trajectory.png` (colored by perceived class), `trajectory.tsv` (pose + feature + `p_pole`/`p_none` + referee + action), env/code snapshot, and sonar dills. Pre-emptive pole-stop avoids overshoot-bumping. Deploy-side edits committed for the trajectory log + stop logic; the 3-class/horizon/p_none/dill/color additions are validated but **uncommitted** as of 2026-06-12 (entangled with the user's live tuning constants).
