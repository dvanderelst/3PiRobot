...
- **Code:** on branch `direct-learning-poletask`. Two-headed inverse on Acq01A+02A+03A, close-range (`MAX_RANGE_MM = 1000`), 4-fold quadrant CV at 60 epochs: class acc 88.9% ± 2.7%, pole recall 81.3% ± 7.5%, pole-az RMSE 15.27° ± 2.32°. Inverse fits cleanly post-calibration overhaul, at least as good as the pre-calibration 2026-05-21 baseline. q=1 spatial region is the persistent weak fold (UQ head correctly signals high σ). Class accuracy plateauing near 89%; pole-az and per-fold σ-tightening still benefit from more data. `SCRIPT_TrainInverseModel.py` refactored 2026-05-29 to run all 4 folds in a single invocation. Natural next moves: additional pole-coverage-focused acquisitions, or drafting the inverse-training Methods subsection in the paper.
