...
- **West-wall discrepancy — diagnose after the next training round.** During Acquisition02 the robot approached the west wall much closer than the plan plot suggested and actually hit it at one waypoint (~y = −1300 mm). The pattern is localised to the west side, so it's almost certainly *not* a planner bug or whole-system issue. Two candidates: (a) digitised west-wall polyline in `arena_features.npz` is offset from the real wall, or (b) homography drift on the camera covering the west (likely `shark`). Cheap diagnostic: park the robot physically touching the west wall at a known y, read the tracker x, compare to the digitised wall x in `arena_features.npz`. The fix will be redoing arena geometry and possibly the per-camera homography for Acq01/02. Holding off until the new Acq03/04 inverse-training pass lands — the wall-depth regression on the west side may be the cleanest place to read the impact.
