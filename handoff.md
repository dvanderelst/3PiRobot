...
- **Code:** on branch `direct-learning-poletask`. **Experiment 1 is ready to run; the inverse now supplies pole RANGE and the robot judges its own arrival (2026-07-29).** The terminal approach no longer uses the tracker: the stop fires when the *perceived* pole range crosses **500 mm**, then the robot regulates the perceived bearing to zero, re-sensing after each correction and giving up after six (`reached_aligned` / `reached_unaligned`). Previously both modalities stopped on tracker geometry, so a sonar run that never perceived the pole could drift within the radius and be scored a success the inverse played no part in. Every run writes `run_summary.json` (outcome, correction count, final perceived class/bearing/range). Trial runner wired (`a67f68a`): set `SENSE_SOURCE` / `POLE` / `START` / `ARENA`, everything else derives — `SESSION` = `direct_<POLE>_S<START>_<source>`, and `CONTROLLER_SEED` from (POLE, START) but *not* the source, so sonar and vision are a matched pair. Two guards: an unrecognised (POLE, START) warns against `pole_positions.json`; an arena whose pole is >400 mm from the intended placement **refuses to run**. **The design:** 5 tape-marked starts; 2 placements **P1 (−1203, −84)** and **P2 (−203, −2084)**, 2236 mm apart; **9 trials** (P1 serves all 5 starts, P2 serves S2–S5) = **18 robot runs**. **Next, in order:** (1) place P1 → snapshot, annotate, build, recalibrate → 5 sonar + 5 vision; then P2 → 4 + 4; (2) Experiment 1 Results. Deploy gate unchanged: re-run `SCRIPT_CalibrateRobot.py` before each placement.
