...
- **Code:** on branch `direct-learning-poletask`. **Experiment 1 is DONE — all 20 robot runs complete (5 starts x 2 pole placements x sonar/vision), written up in the paper in a separate session.** Work now moves to **Experiment 2, path following**, which is *blocked before it can run at all*. Three hard blockers, verified 2026-08-07:
  1. **The training simulator cannot start.** `EnvironmentSimulator.__init__` calls `SonarModel.load`, which expects wall-only `slices_*` artifacts. Those were superseded, their trainer retired in `0ea617e`, and **no archive exists** (`SonarModel_archive/` holds only `2026-06-08_2class` and `2026-07-29_prerange`, both inverse-shaped). `SonarModel/` contains only `inverse_*`. So `SCRIPT_TrainPolicy.py` is dead until the simulator is migrated onto the deployed inverse.
  2. **Nothing survives to run against.** `TargetArenas/` holds only `DirectPole01`/`02`; there is no `Target01`/`Target02` and **zero `target_path.json` anywhere**. Arena, path and both controllers must be rebuilt.
  3. **The decided observation vector is implemented nowhere.** `Library/Policy.py` still has `_OBS_BASE` = 3 wall distances, `_OBS_SIGMA` = 3 sigmas, `_OBS_TAIL` = `prev_rot`; the surviving policy is `in_dim=4`, `[dist_right, dist_center, dist_left, prev_rot]`.
  **The decisions themselves live in `\dnote[18]` in `main.tex` and the body of commit `c7f4cce` — read those first.** In short: the controller takes the *full Experiment 1 local feature including poles* (3 wall distances + sigmas, 3 class probabilities, pole azimuth + sigma, **pole range + sigma**, `prev_rot`; class probabilities gate the pole channels, observation stays fixed-size so no branching); and the step becomes **150 mm** (`Config.fixed_drive_mm`, currently 125) to match Experiment 1's cruise. The real work is the **simulator**, which is an error model rather than an echo simulator: poles need a class confusion model conditioned on true class/range/azimuth plus empirical noise on pole azimuth and range, fitted from the 2135 labelled echoes. Also: clamp range 2000 mm -> the inverse's 1 m abstention horizon (train and deploy clamps must not drift); teacher lookahead 200 -> ~240 mm or re-tune (pure pursuit oscillates as lookahead approaches the step); **the path must pass within ~800 mm of poles** or they are useless as landmarks (recall 94.7% <400 mm but 25% at 800-1000); put the pole channels in the input-ablation battery from the start, since the policy already ignores sigma (`use_sigma=False` in every policy on disk). The blind ablation is the third condition and its plumbing already exists.
