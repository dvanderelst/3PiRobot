...
- **Code:** two-headed inverse on Acq01A + Acq02B, close-range (`MAX_RANGE_MM = 1000`). 4-fold CV: 83% class acc ± 5.9, 88% wall recall, 70% pole recall ± 17, pole precision 71% ± 0.8, pole-az RMSE 17.2°. Pole-az regression head doesn't localise but its *sign* (left/right) is ~68% — keep the regression head, threshold at 0 for sign. **Active calibration thread (2026-05-22 evening, unresolved).** Cameras physically moved + intrinsics redone with peripheral coverage + homography redone with single shared dot board → tiger PnP looks consistent (height 2946 mm vs tape 2945) but cross-camera `dy(x)` diagnostic still U-shaped with ~150–190 mm peak at arena extremes; shark PnP height 3047–3076 mm. Constrained-PnP attempt at fixed height locked the height but didn't shrink the U-shape — confirming PnP is wrong about more than just shark's depth. **Key insight (end of session):** the user's independent ground truth — a laser-aligned floor tape connecting the two camera lens centres — shows the PnP-derived camera nadirs are also off in `(x, y)`, not just `z`. Meanwhile `H_raw` is verifiably correct for both cameras: wall annotations across the two cameras align with each other AND with physical wall positions (user confirmed visually from the stitched arena snapshot). So **the entire feature-plot pipeline (`SCRIPT_TakeEnvSnapshot` → annotation → `SCRIPT_BuildArenaGeometry`) works fine**; only the marker-height correction step (which needs the camera centre `C` and currently gets it from PnP) is broken. **Plan (tomorrow first):** bypass PnP for `C` — drop a plumb line from each camera lens, mark the floor with high-contrast tape, snapshot the arena, read `(Cx, Cy)` for each camera **directly off the stitched-image axes**; combine with tape-measured height to get `C = (Cx, Cy, Cz)` per camera. Small PyLorex change in `Lorex.py` around line 514 to use this measured `C` in the marker-height correction instead of `-R.T @ t`. Verify with `SCRIPT_ProbeTrackerAtWall.py` (robot circle should sit flush against the green wall dots) and `script_check_camera_agreement.py` (U-shape should collapse).
...
- **Calibration state (2026-05-22 evening, unresolved — full plan in "Where to pick up").** Cameras moved + new intrinsics with peripheral coverage + shared-board homography → `H_raw` is good for both cameras (walls project at their true positions and agree across cameras, user verified visually from the stitched snapshot). PnP-derived `R, t` for shark is wrong in `z` *and* in nadir `(x, y)` (laser-tape ground truth shows the crosses in the probe plot don't sit on the lens-centre line). Cross-camera `dy(x)` from `script_check_camera_agreement.py` still U-shaped at ~150–190 mm peak. Constrained-PnP (height-locked) didn't help (RMSE jumped, U-shape barely moved) — PnP's planar-target ambiguity is more than just depth. Tomorrow's path: bypass PnP entirely for the marker-height correction by measuring `C = (Cx, Cy, Cz)` directly (plumb-line + floor markers read off the stitched image).
- **New PyLorex calibration tooling (added 2026-05-22 evening).** `script_collect_intrinsics_live.py` — hands-free auto-capture intrinsic calibration with audio cues, live on-screen coverage map, incremental load-existing-images so you can fill in gaps over multiple sessions; auto-saves `intrinsics_{cam}.yml` + `coverage_{cam}.png`. `script_check_camera_agreement.py` — manual-placement two-camera floor-projection agreement diagnostic, ~15 robot placements with audio cues, outputs CSV + 4-panel plot of `dy(x)`, `dx(y)`, spatial map of |Δ|. `script_run_homography_constrained.py` — PnP with known camera height as a hard constraint via scipy least-squares (Rodrigues + 2D translation, height fixed). Useful when the planar-PnP depth ambiguity is suspected; didn't help in shark's case because the error isn't only in depth.
