...
- **Code:** on branch `direct-learning-poletask`. **Experiment 1 is DONE — all 20 robot runs complete (5 starts x 2 pole placements x sonar/vision), written up in the paper in a separate session.** **Experiment 2 (path following) is now UNBLOCKED**: all three blockers recorded on the morning of 2026-08-07 were cleared the same day (see Code state 2026-08-07). The simulator runs on a fitted inverse *error model*, the policy observation carries the class and pole channels, `default_Path02` is trained, and the deploy chain loads and steps end to end.
  **Next, in order:**
  1. **Recalibrate — but as a DIAGNOSTIC, not just a fix.** The first Path02 robot run (`PolicyRuns/default_Path02_run01`, 2026-08-07 17:24) shows a **-2.66 deg/step yaw residual** over its first 9 full-drive steps. Experiment 1's 20 runs on 2026-07-30 show **+0.52 +/- 0.49** (15 runs with enough full-drive steps; range [-0.58, +1.26]). The Path02 value is **z = -6.5** against that distribution, with **0 of 15** runs close. Ruled out as artifacts: log pairing (both alignments tested; correct one has sd 1.26 vs 8.88), position within run (early vs late differ by only 0.31 deg), rotation chunking (`rotate_in_substeps` caps at 34.5 deg and every Path02 command was inside it, so it would have issued a single call exactly like `RunPolicy` does), gain error (residual uncorrelated with commanded angle, r ~ 0 in both), and run-to-run variance.
     **This is a step change, not gradual staleness** — flat for two months, then a jump in eight days. That shape fits a discrete physical event, not wear, so recalibrating would *absorb* the change rather than explain it. Read the calibration output before accepting it: if curl measures near **-0.038 deg/mm** the robot genuinely changed state (check wheels, debris, battery charge, and anything knocked when the arena was rebuilt for Path02); if it measures near the old **-0.0199** then run01's 9 steps were anomalous and something transient happened during that run. Drive distance is unaffected either way (151.0 +/- 12.6 mm for a commanded 150; scale 1.006).
     **Experiment 1 does NOT need rerunning** (decided 2026-08-07). Its runs were recorded on demonstrably well-behaved hardware — per-run residuals tight around +0.5 deg — so a rerun could only confirm the existing 20/20 or introduce a puzzle, at the cost of robot time plus a Results revision against already-committed numbers. Exp 1 is also closed-loop (the controller re-senses and re-steers each step, so heading error is measured out rather than accumulated); Exp 2's RNN integrates, which is why the same bias matters there and not here.
     The analysis is ~20 lines over `trajectory.tsv` / `step_metrics.tsv` (`rot_deg` vs `diff(yaw_deg)`, restricted to full-drive steps) and is worth repeating on each run's opening steps as a standing check.
  2. **Then the real run.** `SCRIPT_RunPolicy.py` is configured `POLICY=default_Path02`, `ARENA=Path02`, `POLICY_INPUT_SOURCE="live"`, `REPEAT="01"`. It runs 8 preview rollouts and blocks on a y/N prompt first. **The preview cannot tell you anything about calibration** — its `rot_gain ~ U(1+/-0.15)` is a hardcoded synthetic band (`PREVIEW_ROT_GAIN_RANGE_PCT`), never read from `Settings.py`. It measures the policy's *sensitivity* to gain error, not the robot's actual state.
  ...
  4. **Ablations belong on the robot, not in sim** (user's call, 2026-08-07): whether the *simulated* policy degrades without the pole channel says something about the simulator, not about the robot. `use_poles=False` and the blind control are still the intended conditions — the plumbing exists — but run them physically.
