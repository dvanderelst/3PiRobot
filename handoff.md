...
| rule | calls | precision | recall <1 m | recall <1.4 m | false past 1.4 m |
|---|---|---|---|---|---|
| flown model | 77 | 97.4% | ~83% | — (capped) | 0 |
| argmax (as it stands) | 171 | 61.4% | 96.7% | 82.0% | 55 |
| tau 0.70 alone | 75 | 98.7% | 81.1% | 60.7% | 0 |
| **tau 0.50 + agn cap 1000** | 85 | **95.3%** | **87.8%** | 66.4% | **0** |
| **tau 0.60 + agn cap 1400** | 103 | 87.4% | **94.4%** | **73.8%** | 6 |

**`tau 0.70` alone is a trap**: it restores precision by discarding exactly the long-range detections that motivate the re-run — first-detection ranges under it land at 655–1059 mm, essentially where the flown model already was. The agnostic-range veto is what separates the two failure modes, because it polices the class head with the one head that still works out there.

**Two defensible choices.** `tau 0.50 + cap 1000` is strictly better than the flown model — more recall inside 1 m at comparable precision, no far phantoms — and is the low-risk way to put Exp 1 on the current inverse. `tau 0.60 + cap 1400` is the one that buys something new: 74% recall out to 1.4 m where the flown model had none, at 87% precision and 6 far phantoms in 761 steps.
