...
- **Second arena — now Path07, 2026-08-18. NEXT ACTION: deploy `PolicyTraining/default_Path07/best_policy_survival.json` on the robot** (planned 2026-08-19). Trained and evaluated: **67.5% survival over 300 steps against the val-selected artifact's 53.4%** — see Performance notes 2026-08-18 (evening). **Run 300 steps first, not 500**, and check the residual curl in the first ~50 steps (`step_metrics.tsv`): it should come back near Path04's +0.5…+1.8 °/step, and anything near −4.5 means the calibration has not taken. Path07 fails 2.7× more often per lap than Path04, so expect ~50% odds of a clean 500-step run; that is enough to learn from but **not enough for a manipulation series yet**. Path06 is retired to `TargetArenas/old_stuff/`. Three rules that came out of the day and apply to everything downstream: (1) **never judge a policy by `val_mse`**; (2) **check `Settings.py` holds non-identity calibration before every deploy** (`SCRIPT_RunPolicy` now refuses otherwise); (3) **the inverse is blind below ~400 mm**, so the robot has no sonar collision avoidance at contact range. Full detail in Performance notes 2026-08-18 (late). The Path04 landmark series is **not** invalidated by any of this — but do not retrain a Path04 policy without redoing the whole series.
