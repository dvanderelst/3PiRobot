...
- **Code:** two-headed inverse now trained on Acq01A + Acq02B (02B is a 02A redo with the new 81 mm marker; 02A was cut off mid-run), restricted to close-range pings (`MAX_RANGE_MM = 1000` in `SCRIPT_TrainInverseModel.py`). 4-fold CV: 83% class acc ± 5.9, 88% wall recall, 70% pole recall ± 17, pole precision 71% ± 0.8 (cross-fold variance collapsed), pole-az RMSE 17.2°. Pole-az regression head doesn't localise yet, but its *sign* (left/right) is robustly above chance (~68%) — keep the regression head and threshold at 0 for sign; explicit sign-BCE training is worse. **West-wall tracker bias fixed (2026-05-22)** — `floor_xy_mm` now comes from PnP `t_board_tag` instead of the floor-plane homography, with a `height_mm` sanity check against the caliper-measured 150 mm marker plane. Acquisition03/04 collection can now proceed without the systematic position bias. Next: validate the height sanity check on the next acquisition, then keep collecting to tighten pole recall.
...
- **West-wall discrepancy — fixed 2026-05-22 (PyLorex commit `abd3bfa`).** Original bug: during Acquisition02 the robot approached the west wall much closer than the plan plot suggested and hit it at one waypoint (~y = −1300 mm). Root cause was in `PyLorex/LorexLib/Lorex.py`: the tracker reported the marker's `floor_xy_mm` via `pixel_to_board_xy(u, v, use_raw=True)`, which back-projects the marker pixel through `H_raw` — the homography fitted on a calibration board *on the floor*. The marker is physically 150 mm above the floor (caliper-measured 2026-05-22), so this introduced a position bias of `mz / (cz − mz) × distance_from_camera_nadir` pointing **away from the camera's nadir**. Magnitude predicted at ~40–80 mm at typical distances; on the west side that pushed the robot closer to the west wall than the planner intended. Wall polylines were unaffected — they're already back-projected through the wall's top-edge height. Fix applied: when extrinsics are known, `floor_xy_mm` now comes from `t_board_tag[:2]` (PnP's marker position in the board frame), which is unbiased. Field name and downstream consumers unchanged. Homography path retained as the no-extrinsics fallback. Added `Settings.marker_height_mm = 150.0` and `Settings.marker_height_tolerance_mm = 20.0`; a one-line warning prints if PnP's `height_mm` ever deviates from the expected by more than the tolerance, so marker-mount drift or extrinsics regressions become visible. **Validation next acquisition:** confirm no height warnings fire in normal operation; visually confirm the robot plot matches the plan.
